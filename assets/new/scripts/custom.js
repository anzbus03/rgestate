		...
		if(fixed_loaders>0){
			...
			$(window).scroll(function() {
			  if(false==false){
				  ...
				  var  class_e  = loaderElement.attr('class');
			      if(class_e!='loading'){
				  if ($(window).scrollTop() + $(window).height() >= -parseInt(500)+$('#'+ids).offset().top) {
 ...
 **/
 ...
 var resizeTimerInternal ;
 	var lazyBgImg = function( el ) {
											...
											el.css('background-image','url('+el.attr('data-lazy')+')');
											el.removeAttr('data-lazy');
											}
											});
											}
function caroselSingleAfter(){
  
 
   var ajaxloaded  = $('.ajaxLoaded2');
   if(ajaxloaded.length>0){
 
	  $.each(ajaxloaded,function(){
					var Obj = $(this).find(".single-item");
					$(this).removeClass('ajaxLoaded2');
					var e =   $(this).attr('id');
					var   b = true;
