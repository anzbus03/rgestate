(function ($) {
  ...
  function selectToDiv(str) {
    ...
    result = result.replace(/<optgroup[^>]*>/gi, function (matcher) {
      ...
      maingrooupid = (groupId ? groupId[1] : '');
    ...
    result = result.replace(/<option(.*?)<\/option>/gi, function (matcher) {
      ...
      var extendProps = matcher.replace(/data-(\w+)="?(.[^"]+)"?/g, function ($1) {
       ...
       var groupId = /data\-group\-id="(.[^"]*)"(\s|>)/.exec(matcher);
      maingrooupid = (groupId ? groupId[1] : '');
      return '<li data-gid="'+maingrooupid+'" ' + (isDisabled ? ' disabled' : ' tabindex="0"') + ' data-value="' + (value || '') + '" class="dropdown-option ' + (isSelected ? 'dropdown-chose' : '') + '" ' + extendAttr + '>' + (name ? name[1] : '') + '</li>';
    });
    ...
    multiChoose: function (event, status) {
      ...
      $.each(_config.data, function (key, item) {
        ...
        if (item.selected) {
          ...
          _dropdown.name.push('<span class="dropdown-selected" data-region="'+item.groupId+'" >' + item.name + '<i class="del" data-id="' + item.id + '"></i></span>');
        }
      });
  ...
  function Dropdown(options, el) {
   ...
   // this.$select.attr('onchange','showorhide(this)');
