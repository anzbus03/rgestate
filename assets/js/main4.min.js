...
function ajaxSubmitHappenlist(frm, t, i, a) {
	...
	$.ajax({
	  ...
	  success: function(raw) {
		...
		// restore any buttons you disabled...
		if (frm.find("#bb2").length) {
		  btnHtml = frm.find("#bb2").attr("data-html");
		  frm.find("#bb2").prop("disabled", false).html(btnHtml);
		}
  
		if (e.status === "1") {
		  // 1) find the success‐modal and its title element
		  var $modalBody = frm.closest('.modal-body');
		  var $success   = $modalBody.find('.success-modal');
		  var $title     = $success.find('.title');
		  
		  // 2) pull out the HTML template (with {name} & {email})
		  var tpl = $title.html();
		  
		  // 3) replace placeholders with the values from the response
		  tpl = tpl
			.replace('{name}',  e.name  || '')
			.replace('{email}', e.email || '');
		  
		  // 4) write it back into the DOM
		  $title.html(tpl);
  
		  // 5) hide the form, show the success panel
		  frm.closest("#topThirdPlacementLeadFormContainer").hide();
		  frm.closest(".rms-data-h").addClass("hide");
		  $success.addClass("visible");
		} else {
		  // show errors
		  frm.find("#msg_alert").html(e.msg).show();
		  setTimeout(function() { frm.find("#msg_alert").hide() }, 7000);
		}
	  }
	});
  }
