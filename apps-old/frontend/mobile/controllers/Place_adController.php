...
class Place_adController  extends Controller
{
	 ...
	 public function actionCreateAd($user_id=null,$ad_id=null){
        ...
        else
        {
			...
			$optional_array  =  '' ;
			$array_merger_not_hidden = array_merge($not_mandatory_fields,(array) $this->fields_to_hide);
			
		 
			$should_show_fields = $model->should_show_fields();
			
			
			foreach($should_show_fields as $k=>$v){
				 if(in_array($v,(array)$array_merger_not_hidden)){
					 $optional_array .= $v .','  ; 
				 }
			}
			if($optional_array!=''){
				 $optional_array = rtrim($optional_array,','); 
			}
			foreach($should_show_fields as $k=>$v){
				 if(isset($array_merger_not_hidden[$v])){
					 unset($array_merger_not_hidden[$v]);
				 }
			}
			
			foreach($input_data as $k=>$v){
				if(!in_array($v['field_name'],$array_merger_not_hidden) or in_array($v['field_name'],$should_show_fields )){
					 $mandatory_array[]  = $v;
			    }
			}
			 
			echo $this->generateOutPut('SUCCESS','1',array(),array('field_data'=> $mandatory_array,'optional_array'=>$optional_array));
		}
		exit; 
	 }
    ...
    public function actionLoad_location($country_id=null,$query=null,$limit=15,$offset=0){
		 ...
		 foreach($arra as $k=>$v){
			  switch($v->listing_type){
				  case 'C' :
							  ...
							  $return_Array[] = array('title'=> $title ,'section_id'=>'1','listing_type'=>'C','category_id'=>$v->category_id);
							  $title = Yii::t('app','Commercial{t} for Rent  ',array('{t}'=>'->'.$v->category_name));
							  $return_Array[] = array('title'=> $title ,'section_id'=>'2','listing_type'=>'C','category_id'=>$v->category_id);
				  break;
				  case 'R' :
							  $title = Yii::t('app','Residential{t} for Sale',array('{t}'=>'->'.$v->category_name));
							  $return_Array[] = array('title'=> $title ,'section_id'=>'1','listing_type'=>'R','category_id'=>$v->category_id);
							  $title = Yii::t('app','Residential{t} for Rent',array('{t}'=>'->'.$v->category_name));
							  $return_Array[] = array('title'=> $title ,'section_id'=>'2','listing_type'=>'R','category_id'=>$v->category_id);
				  break;
				  case 'B' :
							  $title = Yii::t('app','Residential{t} for Sale',array('{t}'=>'->'.$v->category_name));
							  $return_Array[] = array('title'=> $title ,'section_id'=>'1','listing_type'=>'R','category_id'=>$v->category_id);
	...
	public function actionListPicker($sec_id=null,$listing_type=null,$category_id=null){
			...
			$items = array('value_picker'=>$val_picker,'sec_id'=>$this->checkEmpty($sec_id),'listing_type'=>$this->checkEmpty($listing_type),'category_id'=>$this->checkEmpty($category_id));
