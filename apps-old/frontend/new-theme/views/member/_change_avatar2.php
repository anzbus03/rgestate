 ...
 $resize_height = $options->get('system.upload.avatar_resize_height','100');
	...
	$image_name= ENABLED_AWS_PATH.$model->image ;
	$class="";
	}
...
?>
                ...
                <div class="<?php echo !empty($model->image) ? 'hidden' : '';?>"  id="avatar_updator">
				...
				<?php
				if(!empty($model->image)){
				echo '<div id="imgd" style="width:210px;height:210px;border:1px solid #eee; margin-top:10px; margin-bottom:10px;"><div style="background-image:url('.$model->AvatarUrl.');background-size:ccontain;background-size: contain;width:200px;height:200px;margin:auto;background-repeat:  no-repeat;background-position: center center;" /></div><div class="clearfix"><!-- --></div></div>';
...
<script type="text/javascript">
				var largeImageDropzone = new Dropzone("div#largeImage", { url: "<?php echo Yii::app()->createUrl('place_an_ad/upload',array('width'=>400)); ?>",addRemoveLinks: true, maxFilesize:40 , maxThumbnailFilesize :40 ,timeout: 300000,   resizeWidth :400  ,maxFiles:1,  acceptedFiles: ".gif,.jpg,.jpeg,.png",}) //according to your forms action
				largeImageDropzone.on("removedfile", function(file, serverFileName) {
				$.post("<?php echo Yii::app()->createUrl('place_an_ad/delete_image'); ?>",{file:file.serverId,inp:$("#<?php echo $model->modelName;?>_long_img").val()},function(data){  $("#<?php echo $model->modelName;?>_image").val(data) ;$('#cropEnable').addClass('hidden'); } );
				});
