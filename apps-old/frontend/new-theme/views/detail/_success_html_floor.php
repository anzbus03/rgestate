<style>
   .modal-body {
   
    overflow-x: hidden;
}
</style>
<div class="success-modal florr" style="    margin: auto;">
      
        <div class="anim" style="background:#fff;height:auto; margin:60px 0px 25px;display: block;;">
            <div class="contaiwerwner22" >
           <img src="data:image/svg+xml;base64," style="width: 50px;">
             <div class="clearfix"></div>
            </div>
            <div class="clearfix"></div>
        </div>
        <div class="info">
            <div class="title"><?php echo $this->tag->getTag('your_message_has_been_sent!','Your message has been sent!') ;?></div>
            <div class="text"><?php echo $this->tag->getTag('our_partner_agent_will_get_in_','Our partner agent will get in touch with you soon.') ;?></div>
            <button class="continue"  data-dismiss="modal"><?php echo  $this->tag->gettag('continue','Continue') ;?></button>
        </div>
    </div>
 

