...
class UserController extends Controller
{
   ...
   public $member;
        ...
        if ($request->isPostRequest  ) {
            ...
            if ($model->save()) {
				...
				if ($user->validate()) {
					...
					echo json_encode(array('status'=>'1','after'=>@$_GET['after'],'message'=>$message,'class'=>$class,'id'=>@$_GET['id'] ));Yii::app()->end();
				}
				else
				{ 
					echo json_encode(array('status'=>'0'));Yii::app()->end();
				}
           }
	    } 
	    echo json_encode(array('status'=>'0'));Yii::app()->end();
     ...
     public function actionRemove_properties($id=null){
		...
		if(Yii::app()->user->getId()){
            ...
            Yii::app()->request->cookies[$cookieName2] = $cookie;
                ...
                $data = array(); $cookieName = 'USERFAV'.COUNTRY_ID;
		...
		echo json_encode(array('total_favourite'=>$total_favourite)); exit; 
	}
    public function actionAdd_to_fav(){
					if(!Yii::app()->user->getId()){
					    if(isset($_GET['fun']) and $_GET['fun']=='save_favourite' and !empty($_GET['id'])){
							...
							echo json_encode(array('status'=>'0','message'=>'Your Session Timed Out .Please login.'));Yii::app()->end();
    ...
    public function actionSignin($return=null)
    {
		...
		if(Yii::app()->request->getQuery('a','') == 't'){ $user->need_ajaxlogin = '1';  }
        $model = new ListingUsers();
        $request = Yii::app()->request;
		...
		if(Yii::app()->request->getQuery('t','') == 'c'){ $user->need_captcha_validation = '1';  }
		 if(Yii::app()->user->getId() and !Yii::app()->request->isPostRequest){  $this->redirect(Yii::app()->createUrl('member/dashboard'));    }
      //  print_r($_POST);
        if ($request->isPostRequest && ($attributes = (array)$request->getPost($user->modelName, array()))) {
    ...
    public function actionSignup($return=null,$short_signup=false)
    {
		...
		if(Yii::app()->request->getQuery('t','') == 'c'){ $model->need_captcha_validation = '1';  }
		$this->no_header = '1';
        if (Yii::app()->request->isAjaxRequest and !isset($_GET['_pjax'])) {
			echo CActiveForm::validate($model);
			Yii::app()->end();
		}
          if(Yii::app()->user->getId()){  $this->redirect(Yii::app()->createUrl('member/dashboard'));    }
        if ($request->isPostRequest  ) {
    	...
    	public function actionSuccessfully_regisered($email=null)
	{
	   ...
	   if(!Yii::App()->user->getId()){ 	 $this->redirect(Yii::app()->createUrl('user/signup')); } 	  
 ...
 $model = Yii::app()->user->getModel();
    ...
    {
		 $this->no_header = '1'; ini_set('memory_limit', '-1');
	    if(Yii::app()->user->getId()){  $this->redirect('user/my_profile');    }
        $request = Yii::app()->request;
        $notify = Yii::app()->notify;
    ...
    { 
        if($show=='1'){
         $this->redirect('https://freebites.xyz/user/my_profile');Yii::app()->end();
        }
         if(!Yii::app()->user->getId()){  
	...
	* */
	...
	public function actionEmailVerification($view='verify',$email=null)
	{
	   ...
	   ini_set('memory_limit', '-1');
	     	if(!Yii::app()->user->getId() and empty($email)){ $this->redirect(Yii::app()->createUrl('user/signin')) ; }
	  if(Yii::app()->user->getId() and empty($email)){ $email = base64_encode(Yii::app()->user->getModel()->email); }   	
	...
	public function actionChange_email($view='verify',$email=null)
	{
	  ...
	  if(Yii::app()->user->getId() and empty($email)){ $email = Yii::app()->user->getModel()->email; }   	
...
public function actionDetectlocation()
    ...
    {
       if(!Yii::app()->user->getId()){
		   echo json_encode(array('status'=>'0','message'=>'Your login session timed out. Please Refresh and try again.'));exit;
	   }
	   else{
		   ...
		   echo json_encode(array('status'=>'1','message'=>'Your location updated successfully.'));exit;
	   }
    }
    public function actionResendOtp($phone=null){
                ...
                if (empty($model)) {
							 ...
							 echo json_encode(array('status'=>'0','msg'=> Yii::t('app', 'no user found' )));exit; 
				...
				if(((int)$model->hours_different <  $attemt_last_hrs)  and ((int) $model->otp_attempt >= (int)$attemt )){
					...
					$reset = intdiv($resetminutes, 60).':'. ($resetminutes % 60);
					 echo json_encode(array('status'=>'0','msg'=>Yii::t('app','<span style="line-height:24px;font-size: 14px;">You have reached the limit to resend the code. Please use another method or try again later.    </span>'   )  ));exit; 
				}
                ...
                $out =  $model->getSendOtp();
                	 echo json_encode(array('status'=>'1','msg'=> Yii::t('app', 'Verification Code Sent' )));exit; 
...
public function actionResendOtp2($phone=null){
                ...
                $out =  $model->getSendOtpLogin();
                	 echo json_encode(array('status'=>'1','msg'=> Yii::t('app', 'Successfully Resend Code.' )));exit; 
