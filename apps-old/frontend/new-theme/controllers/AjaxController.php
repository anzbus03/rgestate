...
if(!empty($country_id)){$criteria->condition  .= ' and   t.country_id =  :country   '  ; $criteria->params[':country'] = $country_id ;  }
$criteria->limit = '15'  ; 
$criteria->params[':query'] = '%'.$query.'%' ; 
 if(defined('LANGUAGE')){
...
echo json_encode(array(
	...
	public function fileUploadDropzone(){
			...
			if(isset($_POST['rand'])){
				echo json_encode(array('img'=> $month_folder2. '/' .$img,'rand'=>$_POST['rand'],'file_title'=>Yii::t('app',$file_title,array('_'=>'','-'=>''))));exit;
			} 
