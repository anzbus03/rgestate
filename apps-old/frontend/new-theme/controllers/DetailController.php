...
/*seraching */ 
		$my_views_n  =	array();$total_items_incookie = 6;
		  
		if((isset(Yii::app()->request->cookies['my_views_n'.COUNTRY_ID]))){ $my_views_n  =   Yii::app()->request->cookies['my_views_n'.COUNTRY_ID]->value; 	}
...
/*seraching */ 
  ...
  public function actionPrice_Trends($ytitle=null,$ytitle2=null){
        ...
        if($price_trends_table == '2' or $price_trends_table == '3')
        {
			...
			if(empty($statistics) and $price_trends_table=='3'){ $statistics = PlaceAnAd::model()->pricetrends($_GET); }
        ...
        $statistics = PlaceAnAd::model()->pricetrends($_GET);
		 ...
		 if(empty($statistics)){ echo json_encode(array('status'=>'0')); exit;  }
        $ar_items = array(); 
        if(!empty($statistics)){ 
        ...
        foreach($reverse_array as $month){
         ...
         if(isset($ar_items[$month])){   $itemsArray[$month] = $ar_items[$month];  }else if(isset($ar_items[$previous_month])) {      $itemsArray[$month] = $ar_items[$previous_month];$ar_items[$month] = $ar_items[$previous_month];  } 
