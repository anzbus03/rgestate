<div class="main-content">
	 
  
  <div class="properties" style=" top:0;">
    <div class="container">
      <!-- grid_full_width 4column -->
      <div class="grid_full_width " id="4column" style="  padding-top: 50px;">
        <!--<div class="all-text">
          <h3>Floor Plans</h3>
          
        </div>-->
        <div class="shop-nav clearfix">
          <div class="row">
            <!--<div class="span6">
              <div class="list-grid inleft">
                <ul>
                  <li><a class="active" href="sale.html"><i class="grid4col"></i></a></li>
                
                  <li><a href="listings.html"><i class="grid2list"></i></a></li>
                </ul>
              </div>
            </div>-->
          
              <div class="ordering pull-right ">
              <strong style="color:#000; font-size:20px; font-weight:400;">New Floor Plans</strong></div>
            
          </div>
        </div>
        <div class="row">
			<style>
			table
			{
			border:0px !important;
			}
			table tr th {
			background: #967930 none repeat scroll 0 0;
			color: #fff;
			padding: 10px 0;
			font-size: 16px;
			font-weight: 400;




			}
			td:nth-child(2) {  text-align:center;  }
			td 
			{
			padding: 10px 0;
			border:0px;
			}
			tr
			{
			background: #fff none repeat scroll 0 0;
			border-bottom: 1px solid #dddddd;

			text-indent: 20px;
			font-size: 14px;

			}
			tr:nth-child(even) {
			background: #fff5db none repeat scroll 0 0;
			margin: 0;
			}


.span8 ul {
    font-family: Arial,Helvetica,sans-serif;
    list-style: outside url("../images/floorplan-list.png") disc;
    margin: 10px 0 10px 50px;
}
.span8 ul li {
    
 
}
.span8 ul li a {
    color: #555555;
    display: block;
    font-size: 12px;
    font-weight: bold;
    margin: 5px 0 0;
    text-align: justify;
}
			</style>
        <div class="span8">
              <?php
              if($article)
              {
				  foreach($article as $k=>$v)
				  {
					   if(!empty($v->project))
					  {
					  ?>
					  <div style="width:48%;float:left;padding-right:10px;">
					  <div style="padding:5px 6px;background:#c0c0c0;color:#fff;font-weight:bold;"><?php echo    $v->name;  ?></div>
					  <div>
					 
						  <ul>
							  <?php
							  foreach($v->project as $k2=>$v2)
							  {
								  ?>
								   <li><a href="<?php echo Yii::app()->createUrl($v2->slug.'/floorPlan');?>"><?php echo $v2->project_name;?></a></li>
								  <?
							  }
							  ?>
						 
						  </ul>
						 
					  </div>
					  </div>
					  <?php
				  }
				  }
			  }
			  ?>
            </div>
          <div class="span4">
          <div class="box-siderbar-container">
            <!-- sidebar-box map-box -->
             <?php   $this->widget('frontend.components.web.widgets.subsearch.SubSearchWidget',array('mode'=>""));?>
              </div>
            </div>
            
            <!-- End sidebar-box map-box -->
            
            <!-- sidebar-box our-box -->
            
            <!-- End sidebar-box our-box -->
            
            
            <!-- sidebar-box product_list_wg -->
            <div class="sidebar-box">
                              <?php   $this->widget('frontend.components.web.widgets.relatedproperties.RelatedPropertiesWidget',array('in_array'=>array(),'section_id'=>""));?>
 
            </div>
            <!-- End sidebar-box product_list_wg -->
            
            <!-- sidebar-box searchbox -->
            
            <!-- End sidebar-box searchbox -->
            
          </div>
        </div>
        </div>
        <!-- Page-ination -->
        
        <!-- End Page-ination -->
      </div>
    </div>
  </div>
</div>
