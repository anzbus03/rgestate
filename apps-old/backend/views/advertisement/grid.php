<div id="upate">
<?php 
$hader_text = unserialize($model->header_text) ; 
$image = 'data:image/png;base64,';
 
		$criteria=new CDbCriteria;
		$criteria->select = 't.*, ad.section_id,ad.slug as ad_slug,ban.link_url as banner_slug , art.slug as blog_slug  , ad.ad_description as ad_description, ad.ad_title as ad_title,ban.title as banner_title , art.title as blog_title  , ad.ad_description as ad_description,ban.description as banner_description , art.content as blog_description  ,  (CASE WHEN  t.section = "A" THEN  (SELECT image_name FROM {{ad_image}} img  WHERE  img.ad_id = t.ad_id and  img.status="A" and  img.isTrash="0"  limit 1  )  WHEN t.section = "B" THEN ban.image  ELSE 0 END ) as      ad_image ';
		$criteria->join  .= ' left join {{place_an_ad}} ad ON ad.id = t.ad_id  ';
		$criteria->join  .= ' left join {{banner}} ban ON ban.banner_id = t.banner_id ';
		$criteria->join  .= ' left join {{article}} art ON art.article_id = t.article_id ';
		$criteria->condition  .= ' 1  and  t.layout_id = :layout_id ';
		$criteria->params[':layout_id']   = $model->primaryKey ;
 $data = AdvertisementItems::model()->findAll($criteria);
 $field_array = array();
 if($data){ 
	 foreach($data as $k=>$v){
		  $field_array[$v->row_id] = $v ; 
	 }
 }
 $apps = Yii::app()->apps;
?>
<div class="rw_ab">
<div class="" style="background:#edf0f2;padding:10px;margin-top:0px;margin-bottom:10px;">
<div class="clearfix"></div>	
<div><label>Header For Row1</label><div class="clearfix"></div><input type="text" name="row1" value="<?php echo @$hader_text['row1'];?>" class="form-control rw_1"></div>
<div><label>Sub Header For Row1</label><div class="clearfix"></div><input type="text" name="sub_row1" value="<?php echo @$hader_text['sub_row1'];?>" class="form-control name rw_1"></div>
<div class="clearfix"></div>
<div style="height:5px"></div>
<button type="button" class="btn btn-primary pull-right" onclick="saveHead('rw_1')">Update Headers</button>
<div class="clearfix"></div>
<div style="background:#ccc!important;margin-top:20px;padding:10px;;"><label style="margin-top:0px;font-weight:bold">Add   Row1  Items</label><div class="clearfix"></div> </div>

</div>
<div class="row-lie-1"  style="" >
<?php
$item = 1;
$max_item = $model->max_items;
for($i=$item;$i<=$max_item ;$i++){?>
<div class="col-sm-4 <?php echo isset($field_array[$i]) ? '' : 'empty';?>"   data-id="<?php echo $i;?>" data-number="1" <?php echo !isset($field_array[$i]) ? 'onclick="openPopup(this)"' : '';?>  >
	<?php
	if(isset($field_array[$i])){
		 $v = $field_array[$i] ; 
		$this->renderPartial('_ads',compact('apps','v'));
	}
	else { ?>
	<img src="<?php echo $image;?>" style="height:50px;" />
	<?php }
	$item++;
	?>
</div>
<?php } ?>
</div>
<div class="clearfix"></div>



<div class="rw_ab">
<div class="" style="background:#edf0f2;padding:10px;margin-top:50px;margin-bottom:10px;">
<div class="clearfix"></div>	
<div><label style="margin-top:0px;font-weight:bold">Add Middle Row Items</label><div class="clearfix"></div> </div>
</div>

<div class="col-sm-12 <?php echo isset($field_array[$i]) ? '' : 'empty';?>" data-id="<?php echo $i;?>" data-number="2" <?php echo !isset($field_array[$i]) ? 'onclick="openPopup(this)"' : '';?> >
	<?php
	if(isset($field_array[$i])){
		$v = $field_array[$i];
		$this->renderPartial('_ads_middle',compact('apps','v'));
	}
	else { ?>
	<img src="<?php echo $image;?>" style="height:50px;" />
	<?php } ?>
	</div>
<div class="clearfix"></div>
<div class="" style="background:#edf0f2;padding:10px;margin-top:50px;margin-bottom:10px;">
<div class="clearfix"></div>	
<div><label>Header For Row2</label><div class="clearfix"></div><input type="text" name="row2" value="<?php echo @$hader_text['row2'];?>" class="rw_2 form-control name"></div>
<div><label>Sub Header For Row2</label><div class="clearfix"></div><input type="text" name="sub_row2" value="<?php echo @$hader_text['sub_row2'];?>" class="rw_2 form-control name"></div>
<div style="height:5px"></div>
<button type="button" class="btn btn-primary pull-right" onclick="saveHead('rw_2')">Update Headers</button>
<div class="clearfix"></div>
<div style="background:#ccc!important;margin-top:20px;padding:10px;;"><label style="margin-top:0px;font-weight:bold">Add   Row2  Items</label><div class="clearfix"></div> </div>
</div>
	
<div class="row-lie-1"  >
<?php
$item++;
$max_item = $item+$max_item;
for($i=$item;$i< $max_item ;$i++){?>
<div class="col-sm-4 <?php echo isset($field_array[$i]) ? '' : 'empty';?>" data-id="<?php echo $i;?>" data-number="3"  <?php echo !isset($field_array[$i]) ? 'onclick="openPopup(this)"' : '';?> >
	<?php
	if(isset($field_array[$i])){
		$v = $field_array[$i];
		$this->renderPartial('_ads',compact('apps','v'));
	}
	else { ?>
	<img src="<?php echo $image;?>" style="height:50px;" />
	<?php }
	$item++;
	?>
</div>
<?php } ?>
<div class="clearfix"></div>
</div>

</div>
<div class="clearfix"></div>
 </div>
 
 
 </div>
 
 
