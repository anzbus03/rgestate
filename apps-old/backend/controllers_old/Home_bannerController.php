...
class Home_bannerController extends Controller
{
     ...
     */
     public $Controlloler_title= "Page Banners";
    ...
    public function actionIndex()
    {
         ...
         if($request->getQuery('update_cache','0')=='1'){
                $cacheKey =  'country_banners'.Yii::app()->options->get('system.common.banner_cache','123s421');
                Yii::app()->cache->delete($cacheKey);
                Yii::app()->options->set('system.common.banner_cache',time());
                 $notify->addSuccess(Yii::t('app', 'Cache successfully updated!'));
				  $this->redirect(Yii::app()->createUrl(Yii::app()->controller->id.'/index')) ;
         }
        
         $model = new HomeBanner('serach');
    ...
    public function fileUploadDropzoneBanner(){
			...
			$file =  $_FILES['HomeBanner']['name']['image'];
			//$file =  $_FILES['Banner']['name']['image'];
			$file_orginal = $_FILES['HomeBanner']['tmp_name']['image'];
			...
			$out = $ref->resize($file_orginal,$path,1948,90,$file_format,TRUE);
	...
	public function resetKey(){
			     $cacheKey =  'country_banners'.Yii::app()->options->get('system.common.banner_cache','123s421');
				 Yii::app()->cache->delete($cacheKey);
				 Yii::app()->options->set('system.common.banner_cache',date('YmdHis'));
	}
    public function actionCreate()
    {
        ...
        if ($request->isPostRequest && ($attributes = (array)$request->getPost($model->modelName, array()))) {
            ...
            if( $uploadedFile){
			 ...
			 $uploadedFileMobile=CUploadedFile::getInstance($model,'title');
            if( $uploadedFileMobile){
				 
				$model->title = $this->fileUploadDropzoneBannerMobile();
			}
    ...
    public function actionUpdate($id)
    {
        ...
        $model = HomeBanner::model()->findByPk((int)$id); 
        ...
        if ($request->isPostRequest && ($attributes = (array)$request->getPost($model->modelName, array()))) {
             ...
             $oldfilename =$model->image;$oldmobilefilename =$model->title;
             $model->attributes = $attributes;
            // $model->script =     Yii::app()->params['POST']['script'] ; 
             ...
             $title = CUploadedFile::getInstance($model, 'title');
			 if (is_object($title) && get_class($title)==='CUploadedFile') {
				          
				 
				$model->title = $this->fileUploadDropzoneBannerMobile();
			 } else {
				$model->title = $oldmobilefilename;
			 }
    ...
    public function _setupEditorOptions(CEvent $event)
    {
      ...
      public function fileUploadDropzoneBannerMobile(){
		
			$path_file =  Yii::getPathOfAlias('root.uploads.files');
	 		$ref=new imageResize;
	 		
	 		 
			$file =  $_FILES['HomeBanner']['name']['title'];
			//$file =  $_FILES['Banner']['name']['image'];
			$file_orginal = $_FILES['HomeBanner']['tmp_name']['title'];
			//$file_orginal = $_FILES['Banner']['tmp_name']['image'];
			$ext = pathinfo($file, PATHINFO_EXTENSION);
			$file_format='jpeg';
			if(Yii::App()->isAppName('backend')){
			...
			$out = $ref->resize($file_orginal,$path,540,90,$file_format,TRUE);
			$img = date('Y-m-d-h-i-s').'-'.rand(0,9999).'-'.time().".".$file_format;
