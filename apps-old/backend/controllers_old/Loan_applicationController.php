...
class Loan_applicationController extends Controller
{
     ...
     */
     public $Controlloler_title= "Mortgage Application";
     public $focus = "";
    ...
    public function actionIndex()
    {
         ...
         $condition =  ApplyLoan::model()->search(1);
        $total_application =  ApplyLoan::model()->count($condition);
         
        $model2 = new ApplyLoan();$model2->statusm = 'A';
        $totla_acceptedCriteria =  $model2->search(1);
        $total_accepted_application =  ApplyLoan::model()->count($totla_acceptedCriteria);
         
        
        $model2 = new ApplyLoan();$model2->statusm = 'R';
        $total_rejectedCriteria =  $model2->search(1);
        $total_rejected =  ApplyLoan::model()->count($total_rejectedCriteria);
    ...
    {
        $model = ApplyLoan::model()->findByPk((int)$id);
    ...
    public function actionView($id)
    {
        ...
        $note2 = new MortgageFollowup('search');
      ...
      public function actionBulk_action()
    {
        ...
        if ($action == ApplyLoan::BULK_ACTION_DELETE && count($items)) {
            $affected = 0;
            $customerModel = new  ApplyLoan();
    ...
    {
        $note = MortgageFollowup::model()->findByPk((int)$id);
...
$objPHPExcel->getActiveSheet()->setCellValue('H1', $neModel->getAttributeLabel('monthly_income'));
$objPHPExcel->getActiveSheet()->setCellValue('I1', $neModel->getAttributeLabel('total_loan'));
$objPHPExcel->getActiveSheet()->setCellValue('J1', $neModel->getAttributeLabel('loan_period'.' (yrs)'));
$objPHPExcel->getActiveSheet()->setCellValue('K1', $neModel->getAttributeLabel('interest_rate'));
$objPHPExcel->getActiveSheet()->setCellValue('L1', 'Property URL');
...
$i= 2; 
 foreach($total_result as $res){
	$objPHPExcel->getActiveSheet()->setCellValue('A' . $i, $res->reference)
	                              ...
	                              ->setCellValue('C' . $i,  $res->name)
	                               ->setCellValue('D' . $i,  $res->email) 
	                               ->setCellValue('E' . $i,  $res->phone)
	                               ->setCellValue('F' . $i,  $res->statusTitle)
	                               ->setCellValue('G' . $i, $res->bank->bank_name)
	                               ->setCellValue('H' . $i, $res->monthly_income)
	                               ->setCellValue('I' . $i, $res->total_loan)
	                               ->setCellValue('J' . $i, $res->loan_period)
	                               ->setCellValue('K' . $i, $res->interest_rate)
	                               ->setCellValue('L' . $i, !empty($res->ad_id) ? @$res->ad->DetailUrlAbsolute : '');
	                              $i++;
	                              /*
...
header('Content-Disposition: attachment;filename="mortgageapplication-data-'.date('Ymdhis').'.xlsx"');
