...
class AreaguidesController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $areaguide = new Areaguides('search');
		
         $areaguide->attributes = (array)$request->getQuery($areaguide->modelName, array());
		
        $this->setData(array(
            'pageMetaTitle'     => $this->data->pageMetaTitle . ' | '. Yii::t('areaguides', 'View areaguides'), 
            'pageHeading'       => Yii::t('areaguides', 'View areaguides'),
            'pageBreadcrumbs'   => array(
                Yii::t('areaguides', 'Areaguides') => $this->createUrl('areaguides/list'),
                Yii::t('app', 'View all')
            )
        ));
    ...
    public function actionCreate()
    {
        ...
        if ($request->isPostRequest && ($attributes = (array)$request->getPost($areaguides->modelName, array()))) {
            ...
            if (isset($postData['lifestyle'])) {
                $areaguides->lifestyle = Yii::app()->ioFilter->purify($postData['lifestyle']);
            }
            if (isset($postData['location_detail'])) {
                $areaguides->location_detail = Yii::app()->ioFilter->purify($postData['location_detail']);
            }
            if (!$areaguides->save()) {
        ...
        $this->setData(array(
            ...
            'pageBreadcrumbs'   => array(
                Yii::t('areaguides', 'Areaguides') => $this->createUrl('areaguides/index'),
    ...
    {
        $areaguides = Areaguides::model()->findByPk((int)$id);
    ...
    public function actionDelete($id)
    {
        ...
        if (!$request->getQuery('ajax')) {
            ...
            $this->redirect($request->getPost('returnUrl', array('areaguides/index')));
        }
    }
