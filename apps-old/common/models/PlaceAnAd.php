...
class PlaceAnAd extends ActiveRecord
{
    ...
    public function rules()
    {
		 ...
		 $rules  =  array(
            ...
            array('price', 'validatePrice'),array('salesman_email', 'email'),
            ...
            array('ad_title', 'validateTitle' ),array('contact_person', 'validateContact' ),
		    ...
		    array('price', 'numerical' ),array('preleased,submited_by', 'safe' ),
 ...
 public function validateSectionValue($attribute,$params){
	...
	public function validateSub($attribute,$params){
	          if($this->no_image=='1') { return true; }
	            if(defined('PROSPACE')) { return true; }
 ...
 public function validateSectionValue3($attribute,$params){
  ...
  return 	$_ars;
    ...
    public $advertiser_character;public $advertiser_character_ar;
 ...
 public $custom_price;
	...
	public function getSmallDate() { 
	    if(defined('LANGUAGE') and LANGUAGE=='ar'){ return  Yii::t('app', date('M d,Y',strtotime($this->lastUpdatedn)), array("Jan" => "يناير", "Feb" => "فبراير", "Mar" => "مارس", "Apr" => "أبريل", "May" => "مايو", "Jun" => "يونيو", "Jul" => "يوليو", "Aug" => "أغسطس", "Sep" => "سبتمبر", "Oct" => "أكتوبر", "Nov" => "نوفمبر", "Dec" => "ديسمبر")); }
   ...
   public function getReferenceNumberTitleP() {
	...
	public function getCountryNameSection() { 
	    ...
	    if(empty($this->city_location_latitude) ){ $loc_latitude .=  '<i   class="fa text-red  fa-check-circle"></i>'; }
	    else{ $loc_latitude .=  '<i   class="fa text-green  fa-check-circle"></i>'; }
 ...
 public function getTotal_floorTitle(){
    ...
    public function afterSave(){
			...
			//	$this->package_used = $packagemodel->active_package;
			  ...
			  * */
			  ...
			  /*saving payment */
			  /*saving videos*/$view_360 = 0; $view_video = 0;
			  $post =  Yii::App()->request->getPost('video_urls',array());
   ...
   public function getAllxml($section)
   {
			...
			* */
			...
			if(defined('AREAVALUE') and AREAVALUE != '1'){
				if($this->area_unit == AREAUNIT){    return  Yii::t('app',bcdiv( $this->builtup_area ,1,'2'),array('.00'=>'')).'   '.AREANAME  ; }
				 return       number_format(Yii::t('app',bcdiv( ($this->converted_unit  / AREAVALUE )    ,1,'2'),array('.00'=>'')),0,'.',',').'   '.AREANAME  ;
				  }
			else{
				return    number_format( Yii::t('app',bcdiv( $this->converted_unit ,1,'2'),array('.00'=>'')),0,'.',',').'  Sq. Ft.'  ;
			 }
	    }
	...
	public function getAreaUnit2(){
		if(empty($this->area_unit_1)){ return 'Sq.M.'; }
		else{
	...
	public function getAdTitleWithIcons( )
    {
		...
		if($this->verified=="1"){ $html .=  '<i   class="fa fa-check-circle"></i>'; }
	   ...
	   if(!empty($this->ad_images_g) and !empty($bg)){
			...
			if(!empty($itemsI)){
				...
				if(!empty($bg)){
					...
					else{  $htm = 'data-lazy="'.$apps->getBaseUrl('uploads/files/'.$ad_img).'"'; $c = 'lazy-bg-img'; }
					$html .= '<div class="bg-image '.$c.'" '.$htm.' ></div>';
				}else{
   ...
   * */
   ...
   public function getExpityConditionFronEnd(){
	...
	//	 $criteria->select .=  ' , ct.location_latitude as c_cation_latitude , ct.location_longitude as c_location_longitude  ';
	  ...
	  // print_r($formData);exit;dealer
		...
		if(isset($formData['furnished']) and !empty($formData['furnished'])){
			...
			$criteria->condition .= ' and t.furnished = :fur ';
			$criteria->params[':fur'] = 'Y';
	...
	public function getBathroomTitle(){
	    if($this->bathrooms=='11'){ $this->bathrooms = '10+'; }
		return  $this->bathrooms.'';
	}
	public function getBedroomTitle(){
	    if($this->bedrooms=='11'){ $this->bedrooms = '10+'; }
	    if($this->bedrooms=='15'){ $this->bedrooms = $this->mTag()->getTag('studio','Studio'); }
		return  $this->bedrooms.'';
	}
	public $ad_title2;
	public function getAdTitle2(){
	     if($this->is_rtl($this->ad_title) and LANGUAGE=='en'){
			if(!empty($this->ad_title2)){ return $this->ad_title2; }
	...
	public function getAdDescription2(){
		 if($this->is_rtl($this->ad_description) and LANGUAGE=='en'){
			if(!empty($this->ad_description2)){ return $this->ad_description2; }
		 }
	...
	public function getAreaLocation(){
		 if($this->is_rtl($this->area_location) and LANGUAGE=='en'){
			if(!empty($this->area_location2)){ return $this->area_location2; }
		 }
...
'1010000' =>'1,010,000',
	...
	public function statusArray(){
		 return array("A"=>$this->mTag()->getTag('published','Published'),"I"=>$this->mTag()->getTag('inactive','Inactive'),
	...
	public function getTags(){
		...
		return $html;
			...
			public function listRow1(){
				 $mainRow =  '<span class="property-price new_sec">'.$this->PriceTitleSpanL;if($this->section_id=='2'){ $mainRow .= '<span class="rent_head"> / '.$this->getRentPaidL(1).'</span>'; } $mainRow .= '</span>';if($this->section_id !='3'){  $mainRow .= '<span class="sec_divdr">|</span> <span class="sq_ft">'.$this->BuiltUpArea.'</span>'; } 
				 return $mainRow;					 
	}
	public function listRowPrice(){
	$mainRow =  '<small class="price-se-'.$this->section_id.'" >'.$this->PriceTitleSpanL;if($this->section_id=='2'and empty($this->p_o_r)){ $mainRow .= '<span class="rent_head"> / '.$this->getRentPaidL(1).'</span>'; } $mainRow .= '</small>';  
	return $mainRow;					 
	}
	public function listRowPriceNew(){
	$mainRow =   $this->PriceTitleSpanL2;if($this->section_id=='2'and empty($this->p_o_r)){ $mainRow .= '<span class="rent_head"> / '.$this->getRentPaidL(1).'</span>'; }  ;  
	return $mainRow;					 
	}
...
$w_share_url = Yii::t('app','https://wa.me/{number}?text={text}',array('{number}'=>Yii::t('app',!empty($this->whatsapp) ? $this->whatsapp : $this->mobile_number,array('+'=>'',' '=>'')) ,'{text}'=> $text_message  )); 
   ...
   <div class="buttons smallnomarl footerbtns">
      <div class="col-sm-12  padding-right-0 call-btn-div  mbtn-div" style="padding:0px;width:100% !important">
         ...
         <a type="button" id="fav_button_'.$this->id.'"  class="';$html .= !empty($this->fav) ?  'active' : '';  $html .= '  false  favbtn lastref" onclick="OpenFavouriteNew(this)" data-function="save_favourite" data-id="'.$this->id.'" data-after="saved_fave"    ><i class="fa fa-heart-o"  ></i>  <span class="text-not-mob">Save</span></a>
...
';
        ...
        $email = $this->mTag()->getTag('email','mail');
		 ...
		 $html  = '<div class="d-flex footer-contact"><button type="button"    onclick="OpenCallNewlatest(this)"  data-prop="'. $this->id.'" data-agent="'.$this->OwnerName.'"  data-ref="'.$this->ReferenceNumberTitle.'" data-phone="'.$this->mobile_number.'"  class="btn btnnc btn-transparent"><i class="fa fa-phone"></i> '.$call.'</button>
                  <button type="button" data-reactid="'.$this->id.'" onclick="OpenFormClickNew(this)"    class="btn btnnc btn-transparent"><i class="fa fa-envelope"></i> '.$email.'</button>
                      <a type="button" id="fav_button_'.$this->id.'"  class="'; $html .=!empty($this->fav) ?  'active' : '';  $html .= '  false  favbtn lastref" onclick="OpenFavouriteNew(this)" data-function="save_favourite" data-id="'.$this->id.'" data-after="saved_fave"    ><i class="fa fa-heart-o margin-right-5"  ></i> <span>Save</span></a>
	...
	public function listRow3(){
						 $html = '<h2 class="residential-card__address-heading "><span class="details-link residential-card__details-link"><span class="">';if(!empty($this->area_location)){  $html .= $this->area_location; $html .= ' , ';  }else{ $html .= '';}  if(!empty($this->state_name)){$html .=   $this->state_name ; }   $html .='</span></span></h2>';
						return 	 $html ;		 
	}
  ...
  public function getTagList($check=false){
				...
				$html='';
						if($this->featured=="Y" or  $this->featured2=='Y'){ $html .=  '<li class="list-inline-item featured margin-right-5"><a href="#">'.$this->mTag()->getTag('featured','Featured').'</a></li>'; }
			            if($this->verified=="1"  ){ $html .=  '<li class="list-inline-item verified  margin-right-5"><a href="#" class="" style="color:#fff"><i class="fa fa-check-circle margin-right-2"></i> '.$this->mTag()->getTag('verified','Verified').'</a></li>'; }
			            if($this->hot=="1"  ){ $html .=  '<li class="list-inline-item hot"><a href="#" class="margin-right-2" style="color:#fff">'.$this->mTag()->getTag('hot','Hot').'</a></li>'; }
			 
			    
				if(!empty($html)){ return '<ul class="tag mb0">'.$html.'</ul>'; }
   ...
   public function canShowTag()
    ...
    public function getPriceTitleSpanL($code=''){
		if($this->p_o_r=='1'){ return '<span class="pri sec_'.$this->section_id.'"><span class="price-on-request">'.$this->mTag()->getTag('price_on_request','Price on Request').'</span></span>'; }
	...
	public $reference_number; 
	 public function getReferenceNumberTitle(){
	    if(!empty($this->RefNo)){ return $this->RefNo; } 
        	if(empty($this->id)){
        	 $criteria=new CDbCriteria;
 ...
 const BULK_ACTION_TRASH = 'trash';
  const BULK_ACTION_DOWNLOAD = 'download';
	...
	public function getBoost(){
	    if($this->property_status == '1') { return false; }
	    if($this->featured2=='Y'){
			if($this->featured != 'Y'){
...
return $html;
	...
	public function getOptionCls(){
		...
		if($this->property_status == '0'){
			if($this->section_id=='1'){ $title= $this->mTag()->getTag('mark_as_sold','Mark as Sold');}else{$title=$this->mTag()->getTag('mark_as_leased','Mark as Leased');   }
	...
	public function getSoldStatus(){
		if($this->property_status == '1'){
			if($this->lease_status=='1'){$title= $this->mTag()->getTag('leased','Leased');}else{  $title= $this->mTag()->getTag('vacant','Vacant');}
			return $title ; 
			 }
	}
	public function getSoldStatusH(){
		if($this->property_status == '1'){
			...
			if($this->section_id=='1'){ $title= '<span class="bg-green">Sold</span>';}else{$title= '<span class="bg-green">Leased</span>'; }
			return $title ; 
			 }
	}
 ...
 public function category_list()
	...
	public function getAdTitleArabic(){
	    if(!empty($this->ad_title2) and $this->is_rtl($this->ad_title2) ){  return $this->ad_title2; 	} 
	    return   $this->ad_title ;	 
	}
	...
	public function getAdMetaDescriptionEnglish(){
	    if(!empty($this->ad_description2) and !$this->is_rtl($this->ad_description2) ){  return StringHelper::truncateLength($this->ad_description2, 160 ); }	 
		return StringHelper::truncateLength($this->ad_description, 160 );
	}
	public function getMetaTitleEnglish(){
		if(!empty($this->meta_title)){ 	return $this->meta_title; }
		else{ return  $this->adTitleEnglish; }
		
	}
	public function getMetaTitleArabic(){
		if(!empty($this->meta_title_ar)){ return $this->meta_title_ar; }
		else{ return  $this->adTitleArabic; }
		
	}
	
	public function getMetaDescriptionEnglish(){
		if(!empty($this->meta_description) ){ return $this->meta_description; }
		else{ return $this->adMetaDescriptionEnglish; } 
	}
	public function getMetaDescriptionArabic(){
		if(!empty($this->meta_description_ar) ){ return $this->meta_description_ar; }
		else{ return $this->AdMetaDescriptionArabic; } 
	}
	
	public function getAdMetaDescriptionArabic(){
	    if(!empty($this->ad_description2) and $this->is_rtl($this->ad_description2) ){  return StringHelper::truncateLength($this->ad_description2, 150); }	 
		return StringHelper::truncateLength($this->ad_description, 150);
	}
	public $l_slug;
	public function getPriceTitleSimple($code=''){
		 
		if($this->p_o_r=='1'){ return  '0'; }
	...
	public function getPriceTitleSimpleRent($code=''){
		...
		if($this->p_o_r=='1'){ return  $this->mTag()->getTag('price_on_request','Price on Request'); }
	...
	public function getCdateTitle(){
	 if($this->expiry_date=='0000-00-00'){	 return false; }
	 if(!empty($this->expiry_date)){ return date('m/d/y',strtotime($this->expiry_date));}
	 return false;
 ...
 public function generateLinks($formData,$category_text=null){
        ...
        $data =  array_filter($formData);$html = ''; 
            ...
            foreach($region_list as $k=>$v){ 
                ...
                $html .= '<li>'.Chtml::link(Yii::t('app',$titlte,array('{l}'=>$v)),Yii::app()->createUrl('listing/index',$ar)).'<li>';
                	...
                	$criteria->order = 'count(t.id) desc';$criteria->limit = 15; 
