...
class PlaceAnAdSlug extends PlaceAnAd
{ 
		public function slugMaker(){
		$criteria			 	 =	new CDbCriteria; 
		$criteria->condition = 't.section_id in ("1","2","3") and t.isTrash="0" and t.status="A"   ';
	 	$criteria->condition .= ' and CASE WHEN t.slug_cron_updated IS NULL THEN  "1"  ELSE  (CASE WHEN t.user_updated IS NOT NULL THEN TIMESTAMPDIFF(MINUTE,t.user_updated,NOW()) ELSE  0 END ) > 1 END     ';
		...
		$criteria->select = 't.ad_title,tdataad.message as ad_title2,t.id ';
		//$criteria->select = 'TIMESTAMPDIFF(MINUTE,t.date_added,NOW()) as  ad_title ';
		$criteria->order='t.slug_cron_updated IS NULL,t.slug_cron_updated desc';
		$criteria->group = 't.id'; 
    ...
    public function getSlug2($title)
    {
