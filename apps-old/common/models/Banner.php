    ...
    public function relations()
    {
        ...
        return array(
            'position' => array(self::BELONGS_TO, 'BannerPosition', 'position_id','on'=>"position.isTrash='0'",'joinType'=>'INNER JOIN'),
              'listCountries' => array(self::HAS_MANY, 'BannerCountries', 'banner_id'),
...
public function newBannerFn($position='0',$page=false){
		...
		$criteria->params[':date']   = date('Y-m-d') ;
		  	if(defined('COUNTRY_ID')){
			  	$criteria->join  .= ' left join {{banner_countries}} secCoutry on  secCoutry.country_id =  :country  and secCoutry.banner_id = t.banner_id  ';
		...
		if($banner)  {
...

			  {
				  ...
				  break;
				  case  "adImage" :
				  $urlL = !empty($banner->link_url)? Yii::t('app',$banner->link_url,array('/ar/'=>'/'.LANGUAGE.'/','/en/'=>'/'.LANGUAGE.'/')) : 'javascript:void(0)';
				  return '<a href="'.$urlL.'" target="_blank" style="display:block;margin:auto;"><img src="'.Yii::App()->apps->getBaseUrl('uploads/banner/'.$banner->image).'" style="display: block;max-width:'.$banner->banner_width.'px;width:100%;margin:auto;"></a><div class="clearfix"></div><div  style="max-width:'.$banner->banner_width.'px;width:100%;margin:auto;"></div>';
				  break;
			  }
		}
		else { return false; }
	...
	public function findPosition($section=null,$listing_type=null,$position=null){
		...
		if($section=='property-for-sale' and $position=='2'){ return $this->newBannerFn($position='23');  }
		if($section=='property-for-sale' and $position=='3'){ return $this->newBannerFn($position='17');  } 
	 
		/*RENT POSITION*/
		if($section=='property-for-rent' and $position=='1'){ return $this->newBannerFn($position='18');  } 
		if($section=='property-for-rent' and $position=='2'){ return $this->newBannerFn($position='24');  } 
		if($section=='property-for-rent' and $position=='3'){ return $this->newBannerFn($position='19');  } 
		
		/*Residential POSITION*/
		if($section=='residential' and $position=='1'){ return $this->newBannerFn($position='31');  } 
		if($section=='residential' and $position=='2'){  return $this->newBannerFn($position='32');  } 
		if($section=='residential' and $position=='3'){ return $this->newBannerFn($position='33');  } 
	 
	 
	 	/*Commercial POSITION*/
		if($section=='commercial' and $position=='1'){ return $this->newBannerFn($position='34');  } 
		if($section=='commercial' and $position=='2'){ return $this->newBannerFn($position='35');  } 
		if($section=='commercial' and $position=='3'){ return $this->newBannerFn($position='36');  } 
		...
		if($section=='business-for-sale' and $position=='2'){ return $this->newBannerFn($position='21');  } 
	 	/*SALE POSITION Detail*/
		if($section=='1' and $position=='1'){  return $this->newBannerFn($position='25');  } 
		if($section=='1' and $position=='2'){ return $this->newBannerFn($position='26');  } 
		if($section=='1' and $position=='3'){ return $this->newBannerFn($position='27');  }
		
		/*Rent POSITION Detail*/
		if($section=='2' and $position=='1'){ return $this->newBannerFn($position='28');  } 
		if($section=='2' and $position=='2'){ return $this->newBannerFn($position='29');  } 
		if($section=='2' and $position=='3'){ return $this->newBannerFn($position='30');  }
   ...
   public function validateCountries($attribute,$params){
	...
	public function afterSave(){
		parent::aftersave();
		if(!$this->isNewRecord){
					BannerCountries::model()->deleteAllByAttributes(array('banner_id'=>$this->primaryKey));
		}
		 
		$cn_model = new BannerCountries();
				if(!empty($this->listing_countries) and $this->show_all== '1'){
					foreach($this->listing_countries as $country){
						$cn_model_new = clone $cn_model;
						$cn_model_new->banner_id    = $this->primaryKey;
