 ...
 */
class CustomerTag extends  ActiveRecord
{
    /**
    ...
    public function rules()
    {
        ...
        return array(
            array('tag_name,tag_sub_title,color_code,now_of_rows,limit_p', 'required'),
    ...
    public function afterSave(){
		parent::afterSave();
		if(!$this->isNewRecord){
			TagTypeCustomer::model()->deleteAllByAttributes(array('tag_id'=>$this->primaryKey));
		}
			
			if(!empty($this->used_in_type)){
				$TagCategory =new TagTypeCustomer();
...
$criteria->order = '-t.priority desc';
	...
	public function listData($third=null){
		 $criteria=new CDbCriteria;
		 $criteria->condition = '1';
		 $criteria->order = '-t.priority desc';
		if(!empty($third)){
			 $criteria->join  .= ' INNER  JOIN {{tag_type_customer}} tt on tt.tag_id =t.tag_id  and tt.type_id = :type '  ;
			 $criteria->params[':type'] = $third;
			 $criteria->order = '-t.priority desc';
		 }
		$criteria->condition .= ' and t.tag_type="C" ';
