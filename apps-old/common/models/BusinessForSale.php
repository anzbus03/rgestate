...
class BusinessForSale extends ActiveRecord
{
	...
	public function rules()
	{
		...
		$rules  =  array(
			...
			array('section_id, category_id, sub_category_id, country, state, city, district,   user_id, priority,    community_id, sub_community_id,RetUnitCategory,income,roi', 'numerical', 'integerOnly' => true),
...
<?
   ...
   * */
	...
	public function findAds($formData = array(), $count_future = false, $returnCriteria = false, $calculate = false, $user_id = false)
	{
		...
		$cat_join = false;
		$states_join = false;
		if ((isset($formData['word']) and !empty($formData['word'])) or (!empty($formData['type_of']) and   @$formData['sec'] != 'new-development')) {
			$lstype_join = true;
			$criteria->join  .= ' left join {{category}} lstype ON lstype.category_id = t.listing_type ';
			$cat_join = true;
	...
	public function getPriceTitleSpanLNee($requst = 'p_o_r', $from_price = null, $to_price = null)
	{
		...
		$from_price = Yii::t('app', $from_price, array('0.00' => ''));
		$to_price = Yii::t('app', $to_price, array('0.00' => ''));
		if (empty($from_price)) {
			return false;
		}
		if (!empty($to_price)) {
