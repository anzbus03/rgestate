 ...
 * @property User $user
 */
class ListingUserPasswordReset extends ActiveRecord
{
    ...
    {
        return '{{listing_user_password_reset}}';
    }
    ...
    {
        $rules = array(
            array('email', 'required',  'message'=>$this->mTag()->gettag('required','Required')),
            array('email', 'email','message'=>$this->mTag()->getTag('enter_a_valid_email_address.','Enter a valid email address.')),
            array('email', 'exist', 'className' => 'ListingUsers', 'criteria' => array('condition' => 'status = :st or status = :sw', 'params' => array(':st' =>'A' , ':sw'=>'W')),  'message'=>$this->mTag()->gettag('your_email_not_registered_with','Your email not registered with us')),
        );
