...
{ 
	...
	// $criteria->condition .= ' and  (t.cron_images is not null or t.cron_images != "")  '; 
		...
		if(isset($formData['furnished']) and !empty($formData['furnished'])){
		      switch($formData['furnished']){
                ...
                $criteria->join  .=   ' LEFT JOIN {{ad_amenities}} adamenities on adamenities.ad_id = t.id   '; 
                $criteria->params[':amlist'] = '293,492'; 
                 $criteria->condition .= ' and ( t.furnished="Y" OR   adamenities.amenities_id in  (:amlist) ) ';
                break;
                case 'unfurnished':
                     $criteria->join  .=   ' LEFT JOIN {{ad_amenities}} adamenities on adamenities.ad_id = t.id  ';
		             $criteria->params[':amlist'] = '293,492'; 
		             $criteria->condition .= ' and  ( t.furnished!="N" OR   adamenities.amenities_id not in  (:amlist) )   ';
