...
$i= 2; 
if(!defined('LANGUAGE')){ define('LANGUAGE', 'ar'); }
if(!empty($ad)){
foreach($ad as $k=>$v){
   
		//	print_r(array_filter($v->attributes));exit;
		//	echo $v->id; echo '<br />';
...
header('Content-Disposition: attachment;filename="'.$file_name_new.'.xlsx"');
   ...
   public function actionIndex()
    {
		...
		foreach( $ads as $k=>$v){
		    ...
		    echo $v->id;echo $v->site; echo $v->user_id;echo $package;echo '<br />';
		}
		exit; 
		*/
        ...
        $model->unsetAttributes();$model->hide_new_development = '1'; 
    ...
    public function actionUnpublished()
    {
        ...
        $model->unsetAttributes();$model->hide_new_development = '1'; 
        $model->attributes = (array)$request->getQuery($model->modelName, array());
        $model->isTrash = '0';$model->unpublished = '1';
        // $model->listing_type = 'C';
        $this->setData(array(
            'pageMetaTitle'     => $this->data->pageMetaTitle . ' | '. Yii::t(Yii::app()->controller->id, "{$this->Controlloler_title} List"), 
            'pageHeading'       => Yii::t(Yii::app()->controller->id, "{$this->Controlloler_title} List"),
		          ...
		          $cacheKey = 'olx-cookie' ;			
                	...
                	if ($items = Yii::app()->cache->get($cacheKey) ) {
	                    ...
	                    if( $found){ 
		                    ...
		                    $this->render('_already_inserted', compact('found'));exit;
...
echo json_encode(array(
 ...
 public function actionImport_stats()
    ...
    {  
        ...
        $request = Yii::app()->request;
        $notify = Yii::app()->notify; $image_array = array(); 
        if($id){
