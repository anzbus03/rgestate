...
class DashboardController extends Controller
{
    public function init()
    {
        ...
        $this->getData('pageScripts')->mergeWith(array(
            ...
            array('src' => AssetsUrl::js('dashboard.js?q=1'))
        ));
        parent::init();
    }
    ...
    public function actionGlance()
    {
       if (!Yii::app()->request->isAjaxRequest) {
        ...
        $criteria=new CDbCriteria;
        $criteria->join  = 'INNER JOIN {{place_an_ad}} ad ON  t.ad_id = ad.id  ' ;
        $criteria->condition  = '1 and t.status = "I" and ad.section_id != "3" and ad.isTrash = "0" ';   
        $agencies            =  AdImage::model()->count($criteria);
        ...
        $customers          = Yii::app()->format->formatNumber($agents);
        ...
        $a_user    = Yii::app()->format->formatNumber($a_user);
