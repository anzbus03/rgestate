...
class CategoryController extends Controller
{
     ...
     */
     public $Controlloler_title= "Property Type";
         ...
         if($request->isPostRequest) {
				...
				if(count($sortOrderAll)>0)
				{
					...
					foreach($sortOrderAll as $menuId=>$message)
					{
						...
						$id = 'Category_category_name_'.$menuId;
        ...
        if ($request->isPostRequest && ($attributes = (array)$request->getPost($model->modelName, array()))) {
            ...
            if($uploadedFile2){	 $fileName2 = 'ac'.rand(100,1000).$uploadedFile2; $model->active_image = $fileName2; }
             ...
             if (!$model->save()) {
				...
				if($uploadedFile){	$path =  Yii::app()->basePath . '/../../uploads';$uploadedFile->saveAs($path.'/category/'. $fileName);}
				if($uploadedFile2){	$path =  Yii::app()->basePath . '/../../uploads';$uploadedFile2->saveAs($path.'/category/'. $fileName2);}
				Yii::app()->options->set('system.common.category_cache',date('YmdHis'));
    ...
    public function actionUpdate($id)
    {
        ...
        if ($request->isPostRequest && ($attributes = (array)$request->getPost($model->modelName, array()))) {
            ...
            if (!$model->save()) {
                ...
                if (is_object($file) && get_class($file)==='CUploadedFile') { $path =  Yii::app()->basePath . '/../../uploads'; $file->saveAs($path.'/category/'. $fileName); 	if ($oldfilename != $fileName) { @unlink(Yii::app()->basePath . '/../../uploads/default/'. $oldfilename); } }  
                if (is_object($file2) && get_class($file2)==='CUploadedFile') { $path =  Yii::app()->basePath . '/../../uploads'; $file2->saveAs($path.'/category/'. $fileName2); 	if ($oldfilename2 != $fileName2) { @unlink(Yii::app()->basePath . '/../../uploads/default/'. $oldfilename2); } }  
				Yii::app()->options->set('system.common.category_cache',date('YmdHis'));
                $notify->addSuccess(Yii::t('app', 'Your form has been successfully saved!'));
            }
