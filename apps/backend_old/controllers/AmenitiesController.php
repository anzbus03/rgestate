...
class AmenitiesController extends Controller
{
     ...
     */
     public $Controlloler_title= "Amenities";
			...
			Yii::import('backend.controllers.TranslateController');
				...
				if(count($sortOrderAll)>0)
				{
					...
					foreach($sortOrderAll as $menuId=>$message)
					{
						...
						$id = 'Amenities_amenities_name_'.$menuId;
						$relation = 'am_id';
						$relationID = $menuId;
						$lan = 'ar';
						if(!empty($message)){
    ...
    public function actionCreate()
    {
        ...
        if ($request->isPostRequest && ($attributes = (array)$request->getPost($model->modelName, array()))) {
            ...
            $attributes2 = (array)$request->getPost('amenities_available_for');
            ...
            if (!$model->save()) {
				...
				if($attributes2)
				{
					$Amenities =  new   AmenitiesCategoryList;
					foreach($attributes2 as $k)
					{
						$Amenities->isNewRecord =true;
						$Amenities->amenities_id = $model->amenities_id ;
						$Amenities->category_id = $k ;
						$Amenities->save();
    ...
    public function actionUpdate($id)
    {
        ...
        $model = Amenities::model()->findByPk((int)$id);
        ...
        if ($request->isPostRequest && ($attributes = (array)$request->getPost($model->modelName, array()))) {
             $model->attributes = $attributes;
             $attributes2 = (array)$request->getPost('amenities_available_for');
             $model->amenities_available_for = $attributes2;
            ...
            if (!$model->save()) {
                ...
                $notify->addError(Yii::t('app', 'Your form has a few errors, please fix them and try again!'));
				   ...
				   $Amenities =  new   AmenitiesCategoryList;
				   $Amenities->deleteAll(array("condition"=>"amenities_id=:amenities","params"=>array(":amenities"=>$model->amenities_id)));
				   if($attributes2)
				   {
					   foreach($attributes2 as $k)
					   {
            ...
            if ($collection->success) {
			 ...
			 $model->amenities_available_for =  CHtml::listData( $model->relatedCategory,'category_id','category_id');
		}
        $this->setData(array(
