...
class Home_insuranceController  extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $condition =  InsuranceForm1::model()->search(1);
        $total_application =  InsuranceForm1::model()->count($condition);
         
        $model2 = new InsuranceForm1();$model2->statusm = 'A';
        $totla_acceptedCriteria =  $model2->search(1);
        $total_accepted_application =  InsuranceForm1::model()->count($totla_acceptedCriteria);
         
         
        $model2 = new InsuranceForm1();$model2->statusm = 'R';
        $total_rejectedCriteria =  $model2->search(1);
        $total_rejected =  InsuranceForm1::model()->count($total_rejectedCriteria);
    ...
    public function actionView($id)
    {
        ...
        $note2 = new InsuranceFollowup('search');
        ...
        if($model->bank_id=='7'){
            $model = Lonsdale::model()->findByPk((int)$id);

        if (empty($model)) {
            throw new CHttpException(404, Yii::t('app', 'The requested page does not exist.'));
        }
            $this->render('form-landsole', compact('model','note','note2'));
        }else{
    ...
    {
        $note = InsuranceFollowup::model()->findByPk((int)$id);
    ...
    public function actionExport($datepicker=null,$status_m=null){
		 ...
		 $newModel = new InsuranceForm1();
		 if(!empty($status_m)){
			$newModel->statusm = $status_m;
		}
		$criteria=   $newModel->search(1);
		...
		$total_result = $newModel->findAll($criteria);
...
$objPHPExcel->getActiveSheet()->setCellValue('I1', $newModel->getAttributeLabel('property_type'));
$objPHPExcel->getActiveSheet()->setCellValue('J1', $newModel->getAttributeLabel('o_status'));
$objPHPExcel->getActiveSheet()->setCellValue('K1', $newModel->getAttributeLabel('c_required'));
$objPHPExcel->getActiveSheet()->setCellValue('L1', $newModel->getAttributeLabel('v_of_content'));
$objPHPExcel->getActiveSheet()->setCellValue('M1', $newModel->getAttributeLabel('v_of_personal'));
$objPHPExcel->getActiveSheet()->setCellValue('N1', 'Property URL');
$objPHPExcel->getActiveSheet()->setCellValue('O1', "Map");
 ...
 foreach($total_result as $res){
	$objPHPExcel->getActiveSheet()->setCellValue('A' . $i, $res->reference)
	                              ...
	                              ->setCellValue('C' . $i,  $res->f_name)
	                               ...
	                               ->setCellValue('I' . $i, @$res->propertyTitle )
	                               ->setCellValue('J' . $i, @$res->o_statusTitle )
	                               ->setCellValue('K' . $i, @$res->c_requiredTitle )
	                               ->setCellValue('L' . $i, @$res->v_of_contentTitle )
	                               ->setCellValue('M' . $i, @$res->V_of_personalTitle )
	                               ->setCellValue('N' . $i, !empty($res->property_id) ? @$res->property->DetailUrlAbsolute : '')
	                               ->setCellValue('O' . $i,  'https://www.google.com/maps/?q='.$res->location_latitude.','.$res->location_longitude);
	                              $i++;
	                              /*
...
header('Content-Disposition: attachment;filename="homeinsurance-data-'.date('Ymdhis').'.xlsx"');
