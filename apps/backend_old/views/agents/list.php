            ...
            if ($collection->renderGrid) {
                $this->widget('zii.widgets.grid.CGridView', $hooks->applyFilters('grid_view_properties', array(
                    ...
                    'columns' => $hooks->applyFilters('grid_view_columns', array(
                        array(
                            'name'  => 'user_name',
                            'value' => '@$data->first_name." ".@$data->last_name." ".$data->IsFeatured' ,
                            'type'=>'raw',
                        ),
                         array(
                            'name'  => 'designation_id',
                            'value' => '@$data->designation->service_name' ,
                            'type'=>'raw',
                            'filter'=>CHtml::listData(AgentRole::model()->listData(),'service_id','service_name')
                        ),
                        ...
                        array(
                            ...
                            'buttons'   => array(
                                'update' => array(
                                    ...
                                    'url'       => 'Yii::app()->createUrl( "agents/update", array("id" => $data->user_id))',
                                    'imageUrl'  => null,
                                    'options'   => array('title' => Yii::t('app', 'Update'), 'class' => ''),
                                ),
                                'delete' => array(
                                    'label'     => ' &nbsp; <span class="fa fa-trash"></span> &nbsp; ', 
                                    'url'       => 'Yii::app()->createUrl("agents/delete", array("id" => $data->user_id))',
                                    'imageUrl'  => null,
 ...
 function  openUp2(k)
    {
		...
		if(property_id !=undefined){
			...
			$.get('<?php echo Yii::app()->createAbsoluteUrl("place_an_ad/get_tag_list2"); ?>',{id:property_id,listing_type:'A'},function(data){ 
				 var data = JSON.parse(data);
				//  alert(data.enabled)
