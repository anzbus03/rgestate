...
<div><label>Header For Row1</label><div class="clearfix"></div><input type="text" name="row<?php echo  $row;?>" value="<?php echo @$hader_text['row'.$row];?>" class="form-control rw_1"></div>
<div><label>Sub Header For Row1</label><div class="clearfix"></div><input type="text" name="sub_row<?php echo  $row;?>" value="<?php echo @$hader_text['sub_row'.$row];?>" class="form-control name rw_1"></div>
<div class="clearfix"></div>
...
<div class="col-sm-12 <?php echo isset($field_array[$i]) ? '' : 'empty';?>"   data-id="<?php echo $i;?>" data-number="1" <?php echo !isset($field_array[$i]) ? 'onclick="openPopup(this)"' : '';?>  >
	<?php
	if(isset($field_array[$i])){
		 $v = $field_array[$i] ; 
		$this->renderPartial('_ads_slider',compact('apps','v'));
	}
	else { ?>
