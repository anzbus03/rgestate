...
class Invoicr extends FPDF_rotation
{
    ...
    public function Header()
    {
        ...
        $this->SetFont($this->font, 'B', 9);
        $this->SetTextColor($this->color[0], $this->color[1], $this->color[2]);
        $this->Cell(32, $lineheight, strtoupper(Yii::t('orders', 'Booking ID')) . ':', 0, 0, 'L');
        $this->SetTextColor(50, 50, 50);
        $this->SetFont($this->font, '', 9);
        $this->Cell(0, $lineheight, $this->order_uid, 0, 1, 'R');
        ...
        if ($this->PageNo() == 1) {
            ...
            if (isset($this->flipflop)) {
                $to = Yii::t('orders', 'Payment to');
                $from = Yii::t('orders', 'Payment from');
                $to = $this->to;
                $from = $this->from;
                $this->to = $from;
                $this->from = $to;
            }
            $this->Cell($width, $lineheight, strtoupper(Yii::t('orders', 'Payment from')), 0, 0, 'L');
            $this->Cell(0, $lineheight, strtoupper(Yii::t('orders', 'Payment to')), 0, 0, 'L');
            $this->Ln(7);
        ...
        if (!isset($this->productsEnded)) {
            ...
            $this->Cell($this->firstColumnWidth, 10, strtoupper(Yii::t('orders', 'Schedule')), 0, 0, 'L', 0);
