...
class FacebookOAuthService extends EOAuth2Service {
	...
	protected function fetchAttributes() {
		$info = (object)$this->makeSignedRequest('https://graph.facebook.com/v2.5/me', array(
			'query' => array(
				'fields' => join(',', array(
					'id',
					'email',
...
$token = json_decode($keys[0]) ;
 
$access_token= $token->access_token;
$expires= $token->expires_in;
 
		 $this->setState('auth_token',$access_token);
		 $this->setState('expires', isset($expires) ? time() + (int)$expires - 60 : 0);
