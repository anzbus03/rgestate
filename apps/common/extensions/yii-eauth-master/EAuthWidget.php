...
class EAuthWidget extends CWidget {
	...
	public function init() {
		...
		if (!isset($this->my_returnUrl)) {
			$this->my_returnUrl = Yii::app()->urlManager->parseUrl(Yii::app()->request);
		}
	}
	...
	public function run() {
		...
		$this->registerAssets();
		Yii::app()->user->setState("redirectUrl", $this->my_returnUrl );
		$this->render('auth', array(
			'id' => $this->getId(),
			'services' => $this->services,
			'action' => $this->action,
			'my_returnUrl' => $this->my_returnUrl,
		));
	}
