...
class BusinessForSale extends ActiveRecord
{
	public $amenities;
	 ...
	 */
	public function validateNonFranchis($attribute, $params)
	{
		if ($this->category_id == '194') {
			return true;
		}
		$requiredArray = array('listing_type', 'ow_type', 'mandate');
		foreach ($requiredArray as $fl) {
			if (empty($this->$fl)) {
				$this->addError($fl,  $this->mTag()->getTag('required', 'Required'));
			}
		}
	}
	...
	public function validateBusisessInfo($attribute, $params)
	{
		if ($this->category_id == '194') {
			return true;
		}
		$requiredArray = $this->businessrequirement();
		foreach ($requiredArray as $fl) {
			if (empty($this->$fl)) {
				$this->addError($fl,  $this->mTag()->getTag('required', 'Required'));
			}
	...
	public function rules()
	{
		...
		$rules  =  array(
			...
			array('price_false', 'validatePriceRevenue'),
			array('price_b', 'validatePriceBusiness'),
			array('price_v', 'validatePriceValuation'), array('salesman_email', 'email'),
			array('f_fee', 'validateFranchisee'),
			...
			array('section_id, category_id, sub_category_id, nested_sub_category, country, state, city, district,   user_id, priority,    community_id, sub_community_id,RetUnitCategory,income,roi', 'numerical', 'integerOnly' => true),
			...
			array('price,income', 'length', 'max' => 14), array('roi', 'length', 'max' => 3),
			...
			array('ad_images_g,category_id,sub_category_id,nested_sub_category,reference_number,no_image,insert_via,slug_z,verified,location,site,available_units,show_expired,n_send_at,city_location_image,package_used,location_latitude,location_longitude,adv_ch,is_mor,rights,r_facade,may_affect,disputes,p_limits,l_no,plan_no,no_of_u,floor_no,unit_no,expiry_date,selling_price,c_date,unpublished,hot', 'safe'),
			array('meta_title,meta_description,tag_list2,image,company_name,video,image_status,uid,property_status,p_o_r,RetUnitCategory,ow_type,mandate,Rent,maid_room,transaction_type,price_to_false,price_b_to,construction_status,price_v_to,p_limits,disputes,c_date,w_for,RentPerMonth,request_r,p_b_r,p_v_r,user_updated,team_manager,custom_price,xml_update_date,price_to', 'safe'),
			array('nearest_metro,nearest_railway,category_name,community_name,country_name,user_name,keyword,maxSqft,minSqft,sort,year_built,floor_plan,listing_type,area_unit,area_unit_1,p_id,p_url,inv_i,fur_i', 'safe'),
			...
			array('id, section_id, category_id, sub_category_id,nested_sub_category, ad_title, ad_description, price, country, state, city, district, mobile_number, bathrooms, bedrooms, user_id, added_date, modified_date, priority, isTrash, status,occupant_status, slug, image, dynamic, dynamicArray, location_latitude, location_longitude, featured, area_location, xml_inserted, xml_pk, xml_type, xml_reference, xml_listing_date, xml_update_date, code, RefNo, PropertyID, community_id, sub_community_id, property_name, builtup_area, PrimaryUnitView,     FloorNo, HandoverDate,     parking,   salesman_email, expiry_date,       mandate, currency_abr, area_measurement, PDFBrochureLink,property_overview,ReraStrNo', 'safe', 'on' => 'search'),
		);
		return $rules;
	}
	...
	public function validatePrice($attribute, $params)
	{
		...
		if (!empty($this->price_to) and $this->price_to <=  $this->price) {
			$this->addError($attribute,  $this->mTag()->getTag('shouldbe _greater', 'Sholud be less than to value range'));
		}
	}
	public function validatePriceRevenue($attribute, $params)
	{
		if ($this->request_r == '1') {
			return true;
		}
		...
		if (!empty($this->price_to_false) and $this->price_to_false <=  $this->price_false) {
			$this->addError($attribute,  $this->mTag()->getTag('shouldbe _greater', 'Sholud be less than to value range'));
		}
	}
	public function validatePriceBusiness($attribute, $params)
	{
		if ($this->p_b_r == '1') {
			return true;
		}
		...
		if (!empty($this->price_b_to) and $this->price_b_to <=  $this->price_b) {
			$this->addError($attribute,  $this->mTag()->getTag('shouldbe _greater', 'Sholud be less than to value range'));
		}
	}
	public function validatePriceValuation($attribute, $params)
	{
		if ($this->category_id == '194') {
			return true;
		}
		...
		if (!empty($this->price_v_to) and $this->price_v_to <=  $this->price_v) {
			$this->addError($attribute,  $this->mTag()->getTag('shouldbe _greater', 'Sholud be less than to value range'));
		}
	}
	public function validateFranchisee($attribute, $params)
	{
		if ($this->category_id == '194' and empty($this->$attribute)) {
			$this->addError($attribute,  $this->mTag()->getTag('required', 'Required'));
		}
	}
	public $no_image;
	...
	public function attributeLabels()
	{
		...
		$_ars =  array(
			...
			'category_id' => $this->mTag()->getTag('category', 'Category'),
			...
			'builtup_area' => $this->mTag()->getTag('land_size', 'Land Size'),
			'area_unit' => 'Unit',
			...
			'listing_type' => $this->mTag()->getTag('property_type', 'Property Type'),
			'ow_type' => $this->mTag()->getTag('Ownership Type', 'Ownership Type'),
			'b_pre' => $this->mTag()->getTag('Business Premises', 'Property Type'),
			'price' => $this->mTag()->getTag('asking_price', 'Asking Price'),
			'currency_id' => $this->mTag()->getTag('currency', 'Currency'),
			'price_false' => $this->mTag()->getTag('revenue', 'Revenue'),
			'price_b' => $this->mTag()->getTag('business_cash_flow', 'Business Cash Flow'),
			'price_v' => $this->mTag()->getTag('business_valuation', 'Business Valuation'),
			'Rent' => $this->mTag()->getTag('Leasehold Rent', 'Leasehold Rent per annum'),
			'mandate' => $this->mTag()->getTag('Premises Details', 'Premises Details'),
			'state' => $this->mTag()->getTag('Location', 'Location'),
			'maid_room' => $this->mTag()->getTag('Expansion Potential', 'Expansion Potential'),
			'transaction_type' =>  $this->mTag()->getTag('Competition / Market', 'Competition / Market'),
			'p_limits' => $this->mTag()->getTag('Reasons for Selling', 'Reasons for Selling'),
			'disputes' => $this->mTag()->getTag('Trading hours', 'Trading hours'),
			'RetUnitCategory' => $this->mTag()->getTag('Employees', 'Employees'),
			'c_date' => $this->mTag()->getTag('Established Year', 'Established Year'),
			'w_for' =>  $this->mTag()->getTag('Support & training', 'Support & training'),
			'construction_status' => $this->mTag()->getTag('Relocatable', 'Relocatable'),
			'RentPerMonth' => $this->mTag()->getTag('Furniture / Fixtures value', 'Furniture / Fixtures value'),
			'interior_size' => $this->mTag()->getTag('Inventory / Stock value', 'Inventory / Stock value'),
			'inv_i' => $this->mTag()->getTag('included in the asking price', 'included in the asking price'),
			'fur_i' => $this->mTag()->getTag('included in the asking price', 'included in the asking price'),
			'f_fee' => $this->mTag()->getTag('Franchise Fee', 'Franchise Fee'),
			'investment' => $this->mTag()->getTag('Investment', 'Investment'),
		);
		//  $_ars = $label2;
		return 	$_ars;
	...
	public function Competitionmarket()
	{
		return array(
			'1' => $this->mTag()->getTag('Na', 'NA'),
			'2' => $this->mTag()->getTag('Normal', 'Normal'),
			'3' => $this->mTag()->getTag('Competitive', 'Competitive'),
		);
	}
	public function genLabelIncome()
	{
		switch ($this->section_id) {
			case '2':
				return $this->mTag()->getTag('net_income_p.a', 'Net Income P.A');
				break;
			case '1':
	...
	public function search($return = false)
	{
		...
		if (!empty($this->show_expired) and $this->show_expired == '1') {
			/*
			    ...
			    */
			$criteria->condition .= ' and t.section_id != 3 and t.cron_expiry !="1" and t.status="A" ';
		}
...
// 		}
	...
	function getBuiltUpArea()
	{
		if (defined('LANGUAGE') and LANGUAGE == 'ar') {
			switch ($this->atitle) {
				case 'Sq. M.':
					$this->atitle =   'متر مربع';
					break;
			}
		}
		if (!empty($this->atitle)) {
			return number_format(Yii::t('app', bcdiv($this->builtup_area, 1, '2'), array('.00' => '')), 0, '.', ',') . ' ' . $this->atitle;
		}
		if (!empty($this->converted_unit)) {

			if (defined('AREAVALUE') and AREAVALUE != '1') {
				if ($this->area_unit == AREAUNIT) {
					return  Yii::t('app', bcdiv($this->builtup_area, 1, '2'), array('.00' => '')) . '   ' . AREANAME;
				}
				return    number_format(Yii::t('app', bcdiv(($this->converted_unit  /  (1 /  AREAVALUE)), 1, '2'), array('.00' => '')), 0, '.', ',') . '   ' . AREANAME;
			} else {
				return    number_format(Yii::t('app', bcdiv($this->converted_unit, 1, '2'), array('.00' => '')), 0, '.', ',') . '  Sq. M.';
			}
		}

		if (!empty($this->builtup_area)) {
			return  number_format(Yii::t('app', bcdiv($this->builtup_area, 1, '2'), array('.00' => '')), 0, '.', ',') . ' ' . $this->AreaUnit;
		}
	}
	public $atitle2;
	function getInteriorSize()
	{
		...
		if (!empty($this->atitle2) and !empty($this->interior_size) and $this->interior_size != '0.00') {
			return number_format(Yii::t('app', bcdiv($this->interior_size, 1, '2'), array('.00' => '')), 0, '.', ',') . ' ' . $this->atitle2;
		}
		if (!empty($this->interior_size) and $this->interior_size != '0.00') {
			return   number_format(Yii::t('app', bcdiv($this->interior_size, 1, '2'), array('.00' => '')), 0, '.', ',') . ' ' . $this->AreaUnit2;
		}
	}
	public $unit_title;
	...
	function optionRange($name, $label, $range)
	{
		$options = "<option value=\"\">$label</option>";
	
		foreach ($range as $value => $text) {
			$selected = Yii::app()->request->getQuery($name) == $value ? "selected=true" : "";
	...
	function bedroomHtml()
	{
		$range = array(
			...
			'9' => '9 Bedrooms',
			'10' => '10 Bedrooms',
			'bedroom-equal-and-more' => '10+ Bedrooms'
		);
	
		return $this->optionRange("bedrooms", "Bedrooms", $range);
	}
	
	function bathroomHtml()
	{
		$range = array(
			'1' => '1 Bathrooms',
			'2' => '2 Bathrooms',
			...
			'9' => '9 Bathrooms',
			'10' => '10 Bathrooms',
			'bathroom-equal-and-more' => '10+ Bathrooms'
		);
	
		return $this->optionRange("bathrooms", "Bathrooms", $range);
	}
   ...
   * */
	...
	public function findAds($formData = array(), $count_future = false, $returnCriteria = false, $calculate = false, $user_id = false)
	{
		...
		$cat_join = false;
		$states_join = false;
		if ((isset($formData['word']) and !empty($formData['word'])) or (isset($formData['sub_category_id']) or isset($formData['nested_sub_category'])) or (!empty($formData['type_of']) and @$formData['sec'] != 'new-development')) {
			$lstype_join = true;
			$criteria->join  .= ' left join {{category}} lstype ON lstype.category_id = t.listing_type ';
			$cat_join = true;
		...
		if (isset($formData['furnished']) and !empty($formData['furnished'])) {
			$criteria->join  .=   ' INNER JOIN {{ad_amenities}} adamenities on adamenities.ad_id = t.id and  adamenities.amenities_id in  (:amlist)';
			switch ($formData['furnished']) {
				case 'furnished':
					$criteria->params[':amlist'] = '293,492';
					break;
				case 'unfurnished':
					$criteria->params[':amlist'] = '437 ';
					break;
				default:
					$criteria->params[':amlist'] = '0';
					break;
			}
		}
		if (Yii::App()->isAppName('frontend')) {
		...
		if(isset($formData['sub_category']) and !empty($formData['sub_category'])){
	        $criteria->join  .= ' left join {{subcategory}} subcat ON subcat.sub_category_id = t.sub_category_id ';
			$criteria->condition .= ' and subcat.slug=:sub_category_id ';
			$criteria->params[':sub_category_id'] = $formData['sub_category'];
		}
		
		if(isset($formData['nested_sub_category']) and !empty($formData['nested_sub_category'])){
	        $criteria->join  .= ' left join {{subcategory}} nestsubcat ON nestsubcat.sub_category_id = t.nested_sub_category ';
		...
		if (isset($formData['minRevenue']) and !empty($formData['minRevenue'])) {

			$criteria->condition .= ' and t.price_false>=:minRevenue ';
			$criteria->params[':minRevenue'] = $formData['minRevenue'];
		}
		if (isset($formData['maxRevenue']) and !empty($formData['maxRevenue'])) {

			$criteria->condition .= ' and t.price_false<=:maxRevenue ';
			$criteria->params[':maxRevenue'] = $formData['maxRevenue'];
		}
		if (isset($formData['minValuation']) and !empty($formData['minValuation'])) {

			$criteria->condition .= ' and t.price_v>=:minValuation ';
			$criteria->params[':minValuation'] = $formData['minValuation'];
		}
		if (isset($formData['maxValuation']) and !empty($formData['maxValuation'])) {

			$criteria->condition .= ' and t.price_v<=:maxValuation ';
			$criteria->params[':maxValuation'] = $formData['maxValuation'];
		}
		...
		if (isset($formData['type_of']) and !empty(array_filter((array)$formData['type_of'])) and is_array($formData['type_of'])) {
			...
			if (strpos($formData['type_of'], '_') !== false) {
				...
				$criteria->condition .= ' and cat.slug =:lstype ';
		...
		switch ($order_val) {
			...
			case 'revenue-desc':
				$order  = 't.price_false desc ';
				break;
			case 'revenue-asc':
				$order  = 't.price_false asc ';
				break;

			case 'cashflow-desc':
				$order  = 't.price_b desc ';
				break;
			case 'cashflow-asc':
				$order  = 't.price_b asc ';
				break;
	...
	public function getDetailUrl()
	{
		...
		//return Yii::app()->createAbsoluteUrl('detail/index_business',array('id'=>$this->id));
		if (defined('LANGUAGE') and LANGUAGE == 'en' and !empty($this->slug_en)) {
			return Yii::app()->createUrl('detail/index_business/section/' . $this->section_id, array('slug_en' => $this->slug_en));
		}
		if (defined('LANGUAGE') and LANGUAGE == 'ar' and !empty($this->slug_ar)) {
			return Yii::app()->createUrl('detail/index_business/section/' . $this->section_id, array('slug_ar' => $this->slug_ar));
		}
	...
	public function getDetailUrlAbs()
	{
		...
		*/
		return Yii::app()->createAbsoluteUrl('detail/index_business', array('id' => $this->id));
		return Yii::app()->createAbsoluteUrl('detail/short_link_business/section/' . $this->section_id, array('id' => $this->id));
	}
	...
	public function sortArray()
	{
		...
		return array(
			...
			//	'sqft-desc'=>$this->mTag()->getTag('size_(low_to_high)','Size (Low to High)'),
			//'sqft-asc'=>$this->mTag()->getTag('size_(high_to_low)','Size (High to Low)'),

			'revenue-desc' => $this->mTag()->getTag('revenue_(high_to_low)', 'Revenue (High to Low)'),
			'revenue-asc' => $this->mTag()->getTag('revenue_(low_to_high)', 'Revenue (Low to High)'),

			'cashflow-desc' => $this->mTag()->getTag('cashflow_(high_to_low)', 'Cashflow (High to Low)'),
			'cashflow-asc' => $this->mTag()->getTag('cashflow_(low_to_high)', 'Cashflow (Low to High)'),

			'valuation-desc' => $this->mTag()->getTag('valuation_(high_to_low)', 'Valuation (High to Low)'),
			'valuation-asc' => $this->mTag()->getTag('valuation_(low_to_high) ', 'Valuation (Low to High)'),
	...
	{
		return array(
			"A" => $this->mTag()->getTag('published', 'Published'), "I" => $this->mTag()->getTag('inactive', 'Inactive'),
			"W" => $this->mTag()->getTag('waiting', 'Waiting'), 'R' => $this->mTag()->getTag('rejected', 'Rejected')
		);
	}
	...
	public function getPriceTitleSpanLNee($requst = 'p_o_r', $from_price = null, $to_price = null)
	{
		if ($this->$requst == '1') {
		...
		$from_price = Yii::t('app', $from_price, array('.00' => ''));
		$to_price = Yii::t('app', $to_price, array('.00' => ''));
		if (empty($from_price)) {
			return false;
		}
		if (!empty($to_price)) {
			$from = $this->getnewFormat($from_price);
			$to   = $this->getnewFormat($to_price);

			$html =  '<small>AED</small>' . $from . ' - <small>AED</small>' . $to;
			return $html;
		} else {
		    
			$from = $this->getnewFormat($from_price);

			$html = '<small>AED</small>' . $from;
			return $html;
		}
	}
...
';
	...
	public function footerLinkNew2()
	{
		...
		return ' <button type="button"    onclick="OpenCallNewlatest(this)"  data-prop="' . $this->id . '" data-agent="' . $this->OwnerName . '"  data-ref="' . $this->ReferenceNumberTitle . '" data-phone="' . $this->mobile_number . '"  class="btn btnnc btn-transparent"><i class="fa fa-phone"></i> ' . $call . '</button>
                  <button type="button" data-reactid="' . $this->id . '" onclick="OpenFormClickNew(this)"    class="btn btnnc sty btn-thm"><i class="fa fa-envelope"></i> ' . $email . '</button>
                    <a class="spnnwhatsapp" type="button"    target="_blank" style="color:#fff"  data-prop="' . $this->id . '"  data-type="W" onclick="OpenWhatsappNew(this)" data-href="' . $w_share_url . '"></a>';
	}
	...
	public function getPriceTitleSpanL($code = '')
	{
		...
		if (defined('SELECTED_CURRENCY') and SELECTED_CURRENCY != '2') {
			$num = Yii::t('app', bcdiv($this->price * SELECT_CURRENCY_RATE, 1, '2'), array('.00' => ''));
		...
		} else if (defined('SYSTEM_CURRENCY') and SYSTEM_CURRENCY == 'usd') {
			...
			$code = $this->currencyTitle;
			$html =  '<span class="pri sec_' . $this->section_id . '"><span class="codc"> ' . $code . ' </span>' . Yii::t('app', $this->getnewFormat($num), array('.00' => '')) . '</span>';
		}
...
// 		return $this->price;
	...
	public function business_descp()
	{
		$interior_size = Yii::t('app', $this->interior_size, array('.00' => ''));
		$interior_size = !empty($interior_size) ? $this->getPriceTitleSpanLNee('category_id', $interior_size, '') : '';

		$RentPerMonth = Yii::t('app', $this->RentPerMonth, array('.00' => ''));
		$RentPerMonth = !empty($RentPerMonth) ? $this->getPriceTitleSpanLNee('category_id', $RentPerMonth, '') : '';

		return
			array(
				'maid_room' => $this->getYesntitleId($this->maid_room),
				'transaction_type' => $this->CompetitionmarketId($this->transaction_type),
				'p_limits' => nl2br($this->p_limits),
				'disputes' => nl2br($this->disputes),
				'RetUnitCategory' => $this->RetUnitCategory,
				'c_date' => $this->c_date,
				'w_for' => $this->getYesntitleId($this->w_for),
				'RentPerMonth' => $RentPerMonth,
				'interior_size' => $interior_size,
				'construction_status' => $this->getYesntitleId($this->construction_status)
			);
	}
	public function property_information()
	{
		$interior_size = Yii::t('app', $this->Rent, array('.00' => ''));
		$rent =  !empty($interior_size) ? $this->getPriceTitleSpanLNee('category_id', $interior_size, '') : '';
		return array(
			'listing_type' =>  !empty($this->listing_category) ? $this->listing_category :  $this->ListingType,
			'ow_type' =>  $this->getMasterVal($this->ow_type),
			'Rent' => $rent,
			'state' => $this->state_name,
			'mandate' => nl2br($this->mandate),
  ...
  <button class="btn btn-secondary slpbtn" type="button" id="dropdownMenuButton' . $this->id . '"  onclick="OpenthiMod(this)">
   ' . $this->mTag()->gettag('boost', 'Boost') . '
	...
	public function getSoldStatus()
	{
		if ($this->property_status == '1') {
			if ($this->section_id == '1') {
				$title = $this->mTag()->getTag('sold', 'Sold');
			} else {
	...
	public function getMetaTitleEnglish()
	{
		if (!empty($this->meta_title)) {
			...
			return  $this->adTitleEnglish . ' - ' . $this->state_name  . ' - ' . $this->ReferenceNumberTitle;
		}
	}
	public function getMetaTitleArabic()
	{
		if (!empty($this->meta_title_ar)) {
			...
			return  $this->adTitleArabic . ' - ' . $this->StatebName . ' - ' . $this->ReferenceNumberTitle;
		}
	}
	...
	{
		if (defined('SELECTED_CURRENCY') and SELECTED_CURRENCY != '2') {
			$num = Yii::t('app', bcdiv($this->income * SELECT_CURRENCY_RATE, 1, '2'), array('.00' => ''));
		} else {
			$num = Yii::t('app', $this->income, array('.00' => ''));
		}
		return    '<small> ' . SELECT_CURRENCY_TITLE . ' </small>' . Yii::t('app', $this->getnewFormat($num), array('.00' => ''));
	}
	...
	public function getYesntitleId($id)
	{
		$ar = $this->YesNoArray2();
		return isset($ar[$id]) ? $ar[$id] : '';
	}
	public function CompetitionmarketId($id)
	{
		$ar = $this->Competitionmarket();
		return isset($ar[$id]) ? $ar[$id] : '';
	}
}
