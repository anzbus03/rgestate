...
//$criteria->condition  .= "  and t.location_longitude != ''   and t.image is null     " ;
     ...
     public function searchLocation($keyword)
	{
	  ...
	  public function getById($city_id=null){
		 $criteria=new CDbCriteria;
		$criteria->condition  = 1 ; 
        $criteria->compare('t.city_id',(int)$city_id);
         if(defined('LANGUAGE')){
	...
	const BULK_ACTION_DOWNLOAD = 'UPDATE_MAP';
	public function getBulkActionsList()
    {
				$ar =   
				array(
							self::BULK_ACTION_DOWNLOAD         => Yii::t('app', 'Update Latitude - Longitude'),
				);
     ...
     public function getById2($city_id=null){
		 $criteria=new CDbCriteria;$criteria->select = 't.city_name,st.state_name';
		$criteria->condition  = 1 ; 
        $criteria->compare('t.city_id',(int)$city_id);
	...
	public function getById3($slug=null){
		 $criteria=new CDbCriteria;$criteria->select = 't.city_name,st.state_name';
		$criteria->condition  = 1 ; 
	 ...
	 public function getCityWithCountry($country_id=null){
		$criteria=new CDbCriteria;
		$criteria->condition = '1';
		$criteria->select = 't.city_name,t.city_id'; 
	 	$criteria->compare('t.country_id',(int)$country_id);
		$criteria->order  = 't.city_name  asc ';
	 
		return  City::model()->findAll($criteria);
	 
	 }
	 public function getCityWithState_22($id)
	{
		 $criteria=new CDbCriteria;
		 $criteria->select="city_name,city_id";
		 $criteria->condition="t.isTrash=0 and state_id=:con";
		 $criteria->params[":con"]= $id ;
		 $criteria->order ="t.city_name";
		 return $this->findAll($criteria);
		 
	}
	 public function getLocationFromCity($city_id=null){
		$criteria=new CDbCriteria;
		$criteria->condition = '1';
		$criteria->select = 't.city_name,t.city_id'; 
	 //	$criteria->compare('t.region_id',(int)$city_id);
		$criteria->order  = 't.city_name  asc '; 
		return  City::model()->findAll($criteria);
