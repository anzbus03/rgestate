...
public function getPrimaryField(){
    ...
    {
        if ($this->isNewRecord && empty($this->template_uid)) {
    ...
    public function findByName($template_name)
    {
        return $this->findByAttributes(array(
            'name' => $template_name,
        ));    
    ...
    public function getTemplateByUid($uid=null){
		 
		$criteria=new CDbCriteria;
		$criteria->select = 't.name,t.template_id,t.receiver_list';
		$criteria->condition = 't.template_uid=:uid';
		if(defined('LANGUAGE')){ $langaugae = LANGUAGE; }
		
		 if(!empty($langaugae) and  $langaugae != 'en'){
			 $criteria->params[':lan'] = $langaugae;
			$criteria->join  .= 'left join `mw_translate` `translate` on (  translate.source_tag = concat("CustomerEmailTemplate_subject_",t.template_id) )          left join `mw_translate_relation` `translationRelation` on translationRelation.template_id = t.template_id  and  translationRelation.translate_id = translate.translate_id  LEFT  JOIN mw_translation_data tdata ON (`translationRelation`.translate_id=tdata.translation_id and tdata.lang=:lan  ) ';
			$criteria->join  .= ' left join `mw_translate` `translate1` on ( translate1.source_tag = concat("CustomerEmailTemplate_content_","",t.template_id) )   left join `mw_translate_relation` `translationRelation1` on translationRelation1.template_id = t.template_id  and  translationRelation1.translate_id = translate1.translate_id  LEFT  JOIN mw_translation_data tdata1 ON (`translationRelation1`.translate_id=tdata1.translation_id and tdata1.lang=:lan  ) ';
			$criteria->distinct = 't.template_id';
			$criteria->select .= ' , CASE WHEN tdata.message IS NOT NULL AND translate.source_tag IS NOT NULL   THEN  tdata.message ELSE t.subject END	 as  subject , CASE WHEN tdata1.message IS NOT NULL AND translate1.source_tag IS NOT NULL  THEN  tdata1.message ELSE t.content END	 as  content ';
		}
		else{
			$criteria->select .= ' ,t.content,t.subject';
		}
