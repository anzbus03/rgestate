 ...
 */
class DeveloperBanner extends ActiveRecord
{
    /**
   ...
   public $country_name;
    public function fetchBanners($browse_country,$default_country)
    {
		...
		if(empty($banners)){
			$banners = $this->country_banners($default_country);
		}
		 return $banners;
	}
	public function listCountryBanners($country){
        ...
        $criteria->compare('f_type','D');
        $criteria->condition .= ' and  ( t.country_id  = :country or  t.country_id is null )  ' ;
	...
	public function country_banners($country){
		...
		$cacheKey =  'home_banners'.$country.Yii::app()->options->get('system.common.cache_id','1');
    ...
    public function search()
    {
        ...
        $criteria=new CDbCriteria;
		$criteria->select = 't.*,CASE WHEN t.country_id is NULL THEN "ALL"  ELSE cn.country_name END  as country_name  ';
        ...
        $criteria->order="cn.country_name asc,-t.priority desc";
