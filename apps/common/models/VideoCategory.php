...
class VideoCategory extends ActiveRecord
{
    ...
    public function rules()
    {
        $rules = array(
            ...
            array('name', 'length', 'max' => 200),
                 array('image', 'file', 'types'=>'jpg,jpeg, gif, png,ico','allowEmpty'=>true),
    ...
    public function relations()
    {
        $relations = array(
           // 'parent' => array(self::BELONGS_TO, 'ArticleCategory', 'parent_id'),
            'categories' => array(self::HAS_MANY, 'VideoCategory', 'parent_id'),
            'articles' => array(self::MANY_MANY, 'Video', '{{video_to_category}}(category_id, article_id)'),
            'blogCount'=> array(self::STAT, 'Video', '{{video_to_category}}(category_id, article_id)','condition'=>'t.status="published"'),
   ...
   public function listData()
    {
		 $criteria = new CDbCriteria();
		 $criteria->condition = ' 1  and status=:status';
		 $criteria->order = ' -t.priority desc , t.category_id asc ';
		  $criteria->params[':status'] =  self::STATUS_ACTIVE;
		 return  $this->findAll($criteria);
	}
}
