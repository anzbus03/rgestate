...
class ThemeHandlerForm extends FormModel
{
 ...
 //print_r($themesInstances);exit;
        $themes = array();
        
        foreach ($themesInstances as $theme) {
            $description = CHtml::encode($theme->description);
            $name = CHtml::encode($theme->name);
            if ($manager->isThemeEnabled($theme->dirName, $appName) && $theme->pageUrl) {
                $name = CHtml::link($name, $theme->pageUrl);
            }
            ...
            $themes[] = array(
                ...
                'canDelete'    => @$theme->_canBeDeleted,
                'canDisable'    => @$theme->_canBeDisabled,
            );
        }
