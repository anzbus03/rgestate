...
class DateTimeHelper 
{
    ...
    public static function timespan($seconds = 1, $time = null)
	{
	  ...
	  public static function timespan2($seconds = 1, $time = null, $limit=null)
	{
		if (!is_numeric($seconds)) {
			$seconds = 1;
		}
		...
		if ($years > 0) {
			$numbers++;
			if(!empty($limit) and $numbers==$limit){
				$str .= Yii::t('app','{n} year|{n} yrs'  ,  $years ) ;
				return $str;
			}
			else{
			$str .= Yii::t('app', '{n} year|{n} yrs' ,   $years ).', ';
		...
		if ($days > 0 || $hours > 0) {
			if ($hours > 0) {
				$numbers++;
				if(!empty($limit) and $numbers==$limit){
					$str .= Yii::t('app',  '{n} hour|{n} hrs.' ,   $hours  );
					return $str;
				}
				else{
					$str .= Yii::t('app',   '{n} hour|{n} hrs.' ,  $hours ).', ';
		...
		if ($days > 0 || $hours > 0 || $minutes > 0) {
			if ($minutes > 0) {
				$numbers++;
				if(!empty($limit) and $numbers==$limit){
					$str .= Yii::t('app',   '{n} minute|{n} mins.' ,  $minutes ) ;
					return $str;
				}
				else{
					$str .= Yii::t('app',   '{n} minute|{n} mins.' ,   $minutes ).', ';
