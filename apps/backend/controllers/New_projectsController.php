...
class New_projectsController   extends Controller
{
	...
	public function actionExportExcel() {
		try {
			// Initialize the model without default attributes
			$model = new NewDevelopment('search');
			...
			$header = array(
				'RefNo', 'Permit No', 'Title', 'Description', 'Section', 'Country', 'City', 'Date Created', 'Type', 'Price', 'Rent', 'Status', 'Category', 'Featured'
			);
   ...
   public function actionIndex()
    {
		...
		if ($model->startDate && $model->endDate) {
			$criteria->addCondition('t.date_added >= :startDate');
			$criteria->addCondition('t.date_added <= :endDate');
			$criteria->params[':startDate'] = $model->startDate;
			$criteria->params[':endDate'] = $model->endDate;
		}
     ...
     public function actionTrash()
    {
         $request = Yii::app()->request;
         $notify = Yii::app()->notify;
         $model = new NewDevelopment('serach');
         if($request->isPostRequest) {
                $sortOrderAll = $_POST['priority'];
				if(count($sortOrderAll)>0)
				{
    ...
    {  
	    ...
	    $this->setData(array(
			'pageMetaTitle'     =>   Yii::t('app', '{name}   :: {p}', array('{name}' => 'Update New Projects ' ,'{p}'=> Yii::app()->options->get('system.common.site_name'))),   
