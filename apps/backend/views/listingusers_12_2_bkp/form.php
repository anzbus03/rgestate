...
if ($viewCollection->renderContent) {
	...
	<div class="clearfix"></div>
		...
		<div class="form-group col-sm-3" hint="">
			...
			<?php 
			echo $form->textField($user , 'phone', array_merge($user->getHtmlOptions('phone'),array("placeholder"=>"Phone Numaber* (+92-3XZ-YYYYYYY)" ,'class'=>'form-control'))); 
			?>
	...
	<div class="clearfix"></div> 
	 ...
	 <div class="clearfix"></div>
	 <div class="openWhenUserType <?php if(!in_array($user->user_type,array('A','C','D'))){ echo 'hide';  };?>">
			<div class="form-group col-sm-6" hint="">
			 
			<?php
		  	 echo $form->labelEx($user, 'company_name'); 
			...
			<?php echo $form->error($user, 'company_name');?>
				 	...
				 	<div class="form-group  col-sm-6">
							...
							<?php
							 echo $form->labelEx($user, 'address');   
		...
		<div class="">
		<?php
		if(!Yii::App()->request->isPostRequest and empty($user->country_id) ){$user->country_id = '66099';}?> 
		<div class="form-group  col-sm-6    " hint="">
		<?php echo $form->labelEx($user, 'country_id');?>
		<?php
		echo $form->dropDownList($user,'country_id', CHtml::listData(Countries::model()->Countrylist(),"country_id" ,"country_name"),array("class"=>"select2", "empty"=>"Select County*",'data-url'=>Yii::App()->createUrl('place_property/select_city_new'),'onchange'=>'load_via_ajax2(this,"state_id")')); 
		?>
	...
	<div class="clearfix"></div>
				...
				<div class="form-group  col-sm-6" hint="">
					...
					<?php echo $form->error($user, 'service_offerng_detail');?>
						...
						echo $form->dropDownList($user, 'mul_state_id', CHtml::listData(States::model()->AllListingStatesOfCountry('66099'),'state_id' , 'state_name') , $user->getHtmlOptions('mul_state_id',array(  'class'=>'select2' ,'style'=>'width:100%;'	,'multiple'=>true	,'data-placeholder'=>'Select Service Areas'	)			)); ?>
		...
		<div class="clearfix"></div>
					...
					if(!$user->isNewRecord and in_array($user->user_type,array('C','D')) and AccessHelper::hasRouteAccess ('listingusers/settings_panel')) { ?> 
									<div class="col-sm-12">
