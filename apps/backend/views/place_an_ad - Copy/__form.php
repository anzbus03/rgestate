    ...
    if ($collection->renderForm) {
        ...
        <div class="card">
            ...
            $(document).ready(function () {
		        ...
		        var city = <?=CJSON::encode( array('0'=>"&nbsp;Select City"));?>; 
                ...
                $('#country').on('change', function (event) {
					...
					if (args) {
						   ...
						   $.post("select_state",{"country":value},function(data){ 
								  ...
								  $("#city").jqxComboBox({ source: city,selectedIndex:0,  displayMember: "name", valueMember: "id", width: '200px', height: '25px' ,});
								  $("#state,#city").jqxComboBox('disableAt', 0 ); 
               ...
               $('#state').on('change', function (event) {
					...
					if (args && !isNaN(parseInt(args.item.value))) {
						   ...
						   $.post("select_city",{"state":value},function(data){ 
			...
			$('#city').on('change', function (event) {
               ...
               $('#section').on('change', function (event) {
				...
				if (args && !isNaN(parseInt(args.item.value))) {
					  $(".btn").attr("disabled");
					  var value = args.item.value;
					  $.post("select_sub_category",{"category":value},function(data){   $("#sub_category_sel").jqxListBox({ theme: 'energyblue',  source: JSON.parse(data) ,displayMember: "name", valueMember: "id"}); $("#sub_category_sel").jqxListBox('disableAt', 0 ); $(".wid2").removeClass("wid3"); } )
				      $(".btn").attr("disabled",true);
					 $("#PlaceAnAd_sub_category_id").val("");
