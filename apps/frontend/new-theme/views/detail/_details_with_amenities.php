...
<div class=" col-sm-12 no-padding-left pull-left spl-no-padding-mob">
	...
	<div style="position:relative;background:#fff !important" class="detail_page_gn d-none-sp"  >
		<div onclick="$('#myModal-nearbyLocation').modal('show');$('#thisschools').click();   " style="height:300px;width:100%; background-position:center;background-image:url('https://maps.googleapis.com/maps/api/staticmap?center=<?php echo $model->location_latitude;?>,<?php echo  $model->location_longitude;?>&zoom=10&size=440x440&scale=16&key=<?php echo $this->options->get('system.common.google_map_api_key','AIzaSyBJ2Jo_mnCk9CnTNbTQAcb__elC9cKt6WQ');?>');"   id="map_canvas3" class="margin-bottom-30">
      ...
      <div class="facts_container">
         ...
         <div class="facts_list ">
            <div class="facts_listitem m-n-displ-h price lefticons hide" style="border-bottom:0px;">
               <div class="facts_label   " style="width:100%"> <?php echo $model->PriceTitleSpanL;?><?php if($model->section_id=='2' and empty($model->p_o_r)){ ?>/<span class="dura"><?php echo $model->getRentPaidL(1); ?> </span> <?php } ?>
           </div>
            ...
            .furnished.lefticons{ background-image: url(../../assets/img/amen_293.svg); }
            </style>
			<?php
			...
			if($model->isPreleased){
    			...
    			if(!empty( $fld)) { 
    			?>
    			<div class="facts_listitem <?php echo $k.' '.$listing_type.' '.$category_ids.' '.$sect_ids;?> <?php echo in_array($k,array('bedrooms','bathrooms','builtup_area','listing_type','category_id','section_id','reference','client_ref','interior_size','l_no','plan_no','no_of_u','floor_no','unit_no','c_date','selling_price','furnished')) ? 'lefticons': '';?>" style="border-bottom:0px;">
                   <div class="facts_label " style="width:100% !important;"><?php echo !in_array($k,array()) ?  $model->getAttributeLabel($k).' <span style="font-weight:500;" dir="auto">'.$fld.'</span>': $fld;?> </div>
...
.sacr .active,.sacr .accordion:hover {
    background-color: var(--secondary-color);
    color: #fff;
}
.sacr button.accordion:hover:after,.sacr button.active.accordion:after{
     color: #fff!important;
}
