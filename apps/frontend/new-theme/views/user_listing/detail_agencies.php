<div class="profile_detail_sec">
  ...
  <?php
	   ...
	   if( strpos( $referer, 'real-estate-agents' ) !== false ) {
			$return_title = 'Back to Agencies';
			$return_url = $referer; 
		} 
		else{
			 $return_title = 'Back To Find Agencies';
    ...
    <div class="row ad-bg margin-bottom-15">
      ...
      <div class="col-md-6 col-sm-6 margin-bottom-15 font-fam">
        <span class="ad-text"><?php echo strtoupper($this->options->get('system.common.site_name','Askaan'));?> LISTING AGENCIES</span>
        <h1 style="margin-left: 15px;"><?php echo $model->fullName;?><br /><small style="font-weight:normall"><?php echo $model->country_name;?></small>
        
        <?php
        if(!empty($model->address)){
         ?>
         <br /><small style="font-weight:normall;font-size:18px"><?php echo $model->address;?></small> 
         <?
        }
        ?>
        </h1>
        ...
        <p class=" margin-left-15 ">
          ...
          <span style="color: #1a777c"><?php echo $model->website;?></span> 
        </p>
        <?php } ?>
        <?php
        if(!empty($model->facebook) or !empty($model->twiter) or !empty($model->google)){ ?> 
        <p class=" margin-left-15 ">
          <?php
          if(!empty($model->facebook)){ ?> 
          <a href="<?php echo $model->facebook;?>" target="_blank" ><i class="fab fa-facebook-f"></i></a>
          <?php } ?> 
             <?php
          if(!empty($model->twiter)){ ?> 
          <a href="<?php echo $model->twiter;?>" target="_blank" class="margin-left-10" ><i class="fab fa-twitter"></i></a>
          <?php } ?> 
              <?php
          if(!empty($model->google)){ ?> 
          <a href="<?php echo $model->google;?>" target="_blank" class="margin-left-10" ><i class="fab fa-google-plus-g"></i></a>
          <?php } ?> 
        </p>
        <?php } ?> 
		<?php /* 
        ...
        * */
        ...
        <div class="col-sm-6 col-md-6 "><span class=""><b>For Sale  :</b> <?php echo $model->sale_total;?></span><br /><?php if(!empty($model->licence_no)){ ?> <b>Agencies License #:</b> <?php echo $model->licence_no;?><?php } ?> </span></div>
        ...
        if(!empty($service_location_country)){ 
        ?>
      
        <p class=" margin-left-15    "><span><b>Service Areas:</b></span> <?php echo   $service_location_country; if(!empty($service_location_state)){ ?>(<?php echo $service_location_state;?>) <?php } ?> </p>
        <?php } ?> 
...
<div class="row" id="" style="width:100%;max-width:800px;margin:auto;">
   <div class="col-sm-12 padding-bottom-40">
    <h1 style="text-align:center;">  A little  bit About  <?php echo $model->fullName;?></h1>
  </div>
...
<div class="row" style="text-align: center;">
  ...
  <div class="  padding-bottom-10" style="margin-top:50px">
    <ul class="nav nav-tabs" id="user_tab_filter"  style="margin:0px;padding:0px;overflow:hidden;width: 100%;margin-bottom: 10px;">
...
<div class="clearfix"></div>
  <div class="col-sm-12 tex-center margin-top-40">
    <h1 style="text-align:center;">  Ask a Question <?php echo $model->fullName;?></h1>
  </div>
