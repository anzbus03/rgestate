...
<div class="row  form-group">
	<div class="col-sm-5 text-right" style="text-align:right !important;">
...
<div class="row form-group">
    ...
    <div class="col-sm-7">
        <?php
        // load all states for the selected country (or empty on first render)
        $states = !empty($model->country)
            ? CHtml::listData(States::model()->findAll([
                  'condition' => 'country_id=:c AND parent_id IS NULL AND isTrash=0',
                  'params'    => [':c' => $model->country],
              ]), 'state_id', 'state_name')
            : [];
        echo $form->dropDownList($model,
            ...
            $states,
            array_merge(
                $model->getHtmlOptions('state'),
                [
                    'empty'    => 'Select Location',
                    'class'    => 'input-text form-control',
                    'data-url' => Yii::app()->createUrl('ajax/select_sublocations'),
                    'onchange' => 'load_via_ajax22(this, "sub_state")',
                ]
            )
        );
        ?>
...
<?php
// 2) SUBLOCATION (child states)
?>
<div class="clearfix"><!-- --></div>
<div class="row form-group">
    <div class="col-sm-5 text-right" style="text-align:right !important;">
        <?php echo $form->labelEx($model, 'Sub location'); ?>
    </div>
    <div class="col-sm-7">
        <?php
        // initially empty or, if editing, load from the current state
        $subs = !empty($model->state)
            ? CHtml::listData(States::model()->findAll([
                  'condition' => 'parent_id=:p AND isTrash=0',
                  'params'    => [':p' => $model->state],
              ]), 'state_id', 'state_name')
            : [];
        echo $form->dropDownList($model,
            'sub_state',
            $subs,
            array_merge(
                $model->getHtmlOptions('sublocation'),
                ['empty' => 'Select Sublocation']
            )
        );
        ?>
        <?php echo $form->error($model, 'sublocation'); ?>
    </div>
</div
<div class="row">
