	...
	<div class="form-group col-sm-12"  >
		<div style="clear:both;padding:5px 5px 5px 0px;color:var(--secondary-color);font-weight:bold;font-size:15px;"><?php echo $this->tag->getTag('enter_business_location','Enter business location');?>.</div>
		<?php
									...
									<div style="position:relative;clear:both;" class="a-loct-c">
									    <label for="BusinessForSale_area_location" style="display:inline;"><div class="leftc"></div></label style="display:inline;"><div class="rightc"></div>
	...
	<style>
	    .overfloe { overflow-y:hidden;height: 100vh;}
	    .zoomer-t img { width:22px; }
	    .zoomer-t { display:none; } .zoomer-t.opn-mp {display:block; }
	    .zoomer-t {     position: absolute;
    right: 0;
    width: 30px;
    height: 30px;
    ...
    box-shadow: 0 1px 6px 0 rgb(32 33 36 / 28%);}
	    .zoomer-l { display:none; }.zoomer-l img { width:30px; height:30px; margin:auto;}
    ...
    box-shadow: 0 1px 6px 0 rgb(32 33 36 / 28%);
	    ...
	    .opened-full .zoomer-t { display:none; }
	    .opened-full #map_canvas { position: fixed !important;
    left: 0;
    right: 0;
    top: 0;
    bottom: 0px;
	...
	<script>
	    function zoomMap(k){
	     $('#map_parent').addClass('opened-full');$('body').addClass('overfloe')
	    }
	    function zoomMapminus(k){
	          $('#map_parent').removeClass('opened-full');$('body').removeClass('overfloe');
	    }
	   
	</script>
	<div  id="map_parent" class="form-group col-sm-12;positio:relative" style="position:relative"><div class="zoomer-t" onclick="zoomMap(this)"><img src="<?php echo Yii::app()->apps->getBaseUrl('assets/img/full-screen.png');?>" /></div><div class="zoomer-l" onclick="zoomMapminus(this)"><img src="<?php echo Yii::app()->apps->getBaseUrl('assets/img/closen.png');?>" class=" "></div>
    ...
    if (componentForm[addressType]) {
      const val = component[componentForm[addressType]];
       placeTitle +=  val +', '; 
		...
		initMap(lat,lng);placeMarker(latlng);
		$('#BusinessForSale_location_latitude').val(lat);
		$('#BusinessForSale_location_longitude').val(lng);
		  if(placeTitle != ''){
   placeTitle = placeTitle.replace(/,\s*$/, "");
 //  $('#BusinessForSale_area_location').val(placeTitle);return false; 
  }
}
</script>
<?php
...
function initAutoCompleteNew(code){}
    function initAutocomplete34() {
        console.log(3);    console.log( '<?php echo COUNTRY_CODE;?>');
	var input = document.getElementById('BusinessForSale_area_location');
	var options = { 
...
<?php }
    ...
    <script>
        ...
        <?php
        $country_code = 'ae'; if(!empty($country_code)){  ?> 
...
?>
	...
	if(url_load !== undefined){
		url_load += '/id/'+$(k).val(); 
		 ...
		 $.get(url_load,function(data){ var data = JSON.parse(data) ;   $('#'+modelName+'_'+id).html(data.data);   })
	}
	}
