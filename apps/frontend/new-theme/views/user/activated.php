<div class="container">
	<div class="clearfix"></div>
	<div class="onlyforpoupuplog">
		<div><img src="<?php echo $this->app->apps->getBaseUrl($this->logo_without); ?>"></div>
	</div>
	<div class="onlyforpoupuplogrelative"></div>
	<div class="clearfix"></div>

	<div class="row justify-content-center text-center">
		<div class="col-xl-12 col-lg-12">


			<style>
				.confirm-page {
					text-align: center;
					margin: 75px 0 60px;

					position: relative;


				}
			</style>
			<h4 class="subheading_font row bold-style mt-5"><?php echo $this->tag->getTag('email_verified!', 'Email Verified!'); ?></h4>
			<div class="panel-wrapper" id="contact">

				<div class="container content-container    ">
					<div class="confirm-page margin-top-20 margin-bottom-20">
						<div class="row">
							<div class="col-sm-12">
								<div><img src="data:image/svg+xml;base64," style=" width:50px;  margin: 0 auto 0px;"></div>

								<h3>
									<font style="vertical-align: inherit;">
										<font style="vertical-align: inherit;"></font>
									</font>
								</h3>
								<p>
									<font style="vertical-align: inherit;">
										<font style="vertical-align: inherit;"><?php echo Yii::t('app', $this->tag->getTag('from_now_on_you_can_get_into_y', 'From now on you can get into your  {p}  account using the login details you have selected.'), array('{p}' => '<b>' . $this->project_name . '</b>')); ?></font>
									</font>
								</p>
								<p> <a href="<?php echo Yii::app()->createUrl('user/signin'); ?>" class="btn btn-primary btn-block headfont" style=" clear: both;border: 0 none;
    height: 40px;
    display: inline-block;
    border-radius: 20px;
    padding: 0 18px;
    font-size: 15px;
    font-weight: 700;
    outline: currentcolor none medium;
    box-shadow: none;
    cursor: pointer;
    margin-top: 10px;
    vertical-align: middle;
    text-align: center;
    background-color: var(--secondary-color);
    color: #fff;
    width: 100%; line-height: 38px;"><?php echo $this->tag->getTag('click_here_to_login', 'Click here to login'); ?></a> </p>

							</div>
						</div>
					</div>
				</div>

				<div class="clearfix"></div>
			</div>

		</div>
	</div>
	<!-- /row -->
</div>