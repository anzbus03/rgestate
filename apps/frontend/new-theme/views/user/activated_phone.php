<div class="container">
					<div class="row justify-content-center text-center">
						<div class="col-xl-12 col-lg-12">
						 
 
	 <style>
	 .confirm-page {
    text-align: center;
    margin: 75px 0 60px;
   
    position: relative;
 
 
}
	 </style>
<h4 class="subheading_font row bold-style">Phone Verified!</h4>	
<div class="panel-wrapper" id="contact">

<div class="container content-container    ">
    	 <div class="confirm-page">
			<div class="row">
				<div class="col-sm-12">
				    	<div><img src="data:image/svg+xml;base64," style=" width:50px;  margin: 0 auto 0px;"></div>
				
				 	<h3><font style="vertical-align: inherit;"><font style="vertical-align: inherit;"></font></font></h3>
					<p><font style="vertical-align: inherit;"><font style="vertical-align: inherit;">From now on you can get into your <b><?php echo $this->project_name;?></b> account using the login details you have selected.</font></font></p>
						<p> <a href="<?php echo Yii::app()->createUrl('user/signin');?>" class="btn btn-primary btn-block headfont" style="font-size: 14px;">Click here to login</a> </p>
				
					</div>
			</div>
		</div>
	  </div>

<div class="clearfix"></div>
 </div>
	 
						</div>
					</div>
					<!-- /row -->
				</div>
