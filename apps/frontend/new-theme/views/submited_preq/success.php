<div class="row justify-content-center text-center">
						<div class="col-xl-12 col-lg-12">
					 
	 <style>
	 .confirm-page {
    text-align: center;
    margin: 75px 0 60px;
   
    position: relative;
 
 
}

	 </style>
	<script>
		//Code template for ec - GTM
		function () {
			return {
				"email": <?php echo $model->email; ?>, 
				"phone_number": <?php echo $model->phone; ?>,
				"address": {
					"first_name": <?php echo $model->name; ?> 
				}
			}
		}
	</script>
<div class="panel-wrapper" id="contact">

<div class="container content-container    ">
    
	 <div class="confirm-page">
			<div class="row">
				<div class="col-sm-12">
					<div><img src="data:image/svg+xml;base64," style=" width:80px;  margin: 0 auto 50px;"></div>
					<h3><font style="vertical-align: inherit;"><font style="vertical-align: inherit;"><?php echo $this->tag->getTag('successfully_submitted!','Successfully Submitted!');?></font></font></h3>
					<p><font style="vertical-align: inherit;"><font style="vertical-align: inherit;"><?php echo Yii::t('app',$this->tag->getTag('your_message_was_successfully_','Your message [email] was successfully sent to the [SITE] Support Team.<br /> One of our representative will contact you soon.'),array('[SITE]'=>$this->project_name, '[email]' => $model->email));?>  </font></font></p>
					<p> <a href="<?php echo Yii::app()->createUrl('site/index');?>" class="btn btn-primary btn-block headfont" style=";;font-size: 14px;line-height: unset;padding: 0px;vertical-align: middle;display: inline-block;padding: 5px 15px;"><?php echo $this->tag->getTag('go_to_the_home_page','Go to the home page');?></a> </p>
					</div>
			</div>
		</div>
	 
 </div>

<div class="clearfix"></div>
 </div>
	 
						</div>
					</div>