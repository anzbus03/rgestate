...
@media only screen and (max-device-width: 480px) {
  ...
  .homepage-main-post {
    ...
    ul.homepage-main-post li {
        ...
        margin: 10px;
        width: calc(33.33% - 20px) !important; /* Three columns on desktop */
        position: relative;
        overflow: hidden;
    }
    ...
    @media screen and (max-width: 768px) {
        ul.homepage-main-post li {
            width: calc(50% - 20px) !important; /* Two columns on iPad screen size */
        }
    }

    @media screen and (max-width: 576px) {
        ul.homepage-main-post li {
            width: 100%  !important; /* One column on phone screen size */
        }
    }
...
<section id="main-content" class="clearfix">
    <div class="container" id="d_column">
        <?php 
        $processedStates = array();
        foreach ($areaguides as $element) {
            $result[$element['area']][] = $element;
        }
        
        foreach ($result as $key => $areas) : 
            if (!in_array($key, $processedStates)) {
                // Process the state only if it hasn't been processed before
                $processedStates[] = $key;
            } else {
                // Skip processing this state as it has already been processed
                continue;
            }
        ?>
            <div class="block-head">
                <h2 class="post-category-title"><a href="#"><?php echo $this->getAreaText($key) ?></a></h2>
            </div>
            
            <ul class="homepage-main-post single-posts" style="margin-bottom: 50px;">
                <?php foreach ($areas as $area) : ?>
                    <?php 
                    // Check if the state name has already been processed
                    if (in_array($area->state_name, $processedStates)) {
                        continue; // Skip this iteration if state name is duplicated
                    }
                    
                    // Add the state name to the processed states array
