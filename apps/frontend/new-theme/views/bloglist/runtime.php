 		...
 		<li class="nsdd <?php if ((int)($k + 1) % 3 == 0  and $k != 0) {
 			...
 			<div class="mainnewscol radius3">
 				<div class="newscontnt">
 					<div>
 						<a title="<?php echo $v->title; ?>" href="<?php echo Yii::app()->createUrl($v->slug . '/blog'); ?>"
 							rel="nofollow">
 							<?php
 							...
 							<img alt="<?php echo $v->title; ?>"
 								...
 								class="PaddingTwo jqlazy" style="display: block;height:202px;"></a>
 					</div>
 					<div class="titlebox titlelength">
 						<h3><a title="<?php echo $v->title; ?>" href="<?php echo Yii::app()->createUrl($v->slug . '/blog'); ?>"
 								class="LHight16"
 								style="float: none; position: static;"><?php echo (strlen($v->title) > 100) ? substr($v->title, 0, 100) . '...' : $v->title; ?></a>
 						</h3>
 						<div class="newsdatetime">
 							<?php echo $commonModel->TranslatingDate(date('M d , Y', strtotime($v->date_added)), $langaugae); ?>
 						</div>
 						<p>
 							<?php
 				...
 				<div class="newsdetailbg clearfix">
 					<ul>
 						<li class="first">
 							<div class="newsview"> <span class="viewico2 sprite"></span> <?php echo $v->viewCount; ?> <span
 									class="text12"><?php echo  $views_title; ?></span></div>
 						</li>
 						<li class="last">
 							<div class="newsview"> <a href="<?php echo Yii::app()->createUrl($v->slug . '/blog'); ?>"
 									title="Read More"><span class="redmorico2 sprite"></span><?php echo  $read_title; ?></a>
 							</div>
 						</li>
 					</ul>
 				</div>
