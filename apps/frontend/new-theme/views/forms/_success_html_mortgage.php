<style>
   .modal-body {
   
    overflow-x: hidden;
}
</style>

<div class="success-modal" style="    margin: auto;" >
        <div class="anim" style="background:#fff;height:auto; margin:60px 0px 25px;display: block;;">
            <div class="contaiwerwner22" >
           <img src="data:image/svg+xml;base64," style="width: 50px;">
             <div class="clearfix"></div>
            </div>
            <div class="clearfix"></div>
        </div>
        <div class="info">
            <div class="title"><?php echo 'Thank you for your submission.' ;?></div>
            <div class="text"><?php echo 'We will get back to you soon.' ;?></div>
            <a href="<?php echo Yii::app()->createUrl('forms/mortgage');?>" class="continue" style="display: inline-block;"  data-dismiss="modal"><?php echo 'Continue' ;?></a>
        </div>
    </div>
 
