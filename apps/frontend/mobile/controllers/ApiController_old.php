...
class ApiController  extends Controller
{
    ...
    public function actionState_list($country_id=null,$limit=0)
     {
		...
		if($listStates){
			foreach($listStates as $k=>$v){
			    ...
			    $icon  = Yii::app()->apps->getBaseUrl('uploads/images/'.$icon,true) ;
    ...
    public function actionFeaturedAgentsList($country_id=null,$state_id=null,$limit=10,$offset=0)
     {
                $agentData = array();
                if(!empty($state_id)){
				...
				if(!empty($field)){	
				    ...
				    foreach($field as $k=>$v){
				       $agent_list[] = array( 'user_id'=>$v->user_id,'designation'=>$this->checkEmpty($v->designation),'nationality'=>$this->checkEmpty($v->country_name),'full_name'=>$v->fullName , 'image' => Yii::app()->apps->getBaseUrl('uploads/images/'.$v->image,true) )  ;
				    }
     ...
     public function actionFeaturedDevelopersList($country_id=null,$state_id=null,$limit=10,$offset=0)
     {
                $agentData = array();
                if(!empty($state_id)){
                $agentData['agent_regi'] = $state_id ;
				...
				if(!empty($field)){	
				    $agent_list = array();
				    foreach($field as $k=>$v){
				       $agent_list[] = array( 'user_id'=>$v->user_id,'full_name'=>$v->fullName , 'image' => Yii::app()->apps->getBaseUrl('uploads/images/'.$v->image,true) )  ;
				    }
    ...
    public function actionFeaturedAgenciesList($country_id=null,$state_id=null,$limit=10,$offset=0)
     {
                $agentData = array();
                if(!empty($state_id)){
                $agentData['agent_regi'] = $state_id ;
				...
				if(!empty($field)){	
				    $agent_list = array();
				    foreach($field as $k=>$v){
				       $agent_list[] = array( 'user_id'=>$v->user_id,'full_name'=>$v->fullName,'nationality'=>$this->checkEmpty($v->country_name) , 'image' => Yii::app()->apps->getBaseUrl('uploads/images/'.$v->image,true) )  ;
				    }
   ...
   public function actionAdsList($sec=null,$limit=10,$offset=0)
     ...
     public function actionSearch_parameters()
     {
        ...
        $array[] =array('title'=>'bedrooms','field_name'=>'bedrooms','val_array'=>$rooms );
        $array[] =array('title'=>'bathrooms','field_name'=>'bathrooms','val_array'=>$bath_rooms );
        $array[] =array('title'=>'Category','field_name'=>'type_of','val_array'=> $categories);
        $array[] =array('title'=>'Listing Type','field_name'=>'listing_type','val_array'=>$listing_type);
        
        $array[] =array('title'=>'Price (Min)','field_name'=>'minPrice','val_array'=>$price_array);	 
        $array[] =array('title'=>'Price (Max)','field_name'=>'maxPrice','val_array'=>$price_array);
        $array[] =array('title'=>'Rental Terms','field_name'=>'rent_paid','val_array'=>array("monthly"=>"Monthly","yearly"=>"Yearly") );
        
        $array[] =array('title'=>'Square Feet (Min)','field_name'=>'minSqft','val_array'=>'');
        $array[] =array('title'=>'Square Feet (Max)','field_name'=>'maxSqft','val_array'=>'');
        
        $array[] =array('title'=>'Keyword','field_name'=>'keyword','val_array'=>'');
        
        $array[] =array('title'=>'Sort By','field_name'=>'sort','val_array'=> $sortArray);
        

		echo $this->generateOutPut('SUCCESS','1', '' , $array);
	    Yii::app()->end();
    } 
    public function actionAgentDetails($agent_id=null){
        ...
        public function actionAgenciesDetails($agency_id=null){
            $criteria=new CDbCriteria;
			$criteria->select = 't.*,cn.country_name
			,(select COALESCE(count(ad.id),0) from {{place_an_ad}} ad where t.user_id = ad.user_id and ad.status="A" and ad.isTrash="0" and ad.section_id='.PlaceAnAd::SALE_ID.' ) as sale_total
			,(select COALESCE(count(ad.id),0) from {{place_an_ad}} ad where t.user_id = ad.user_id and ad.status="A" and ad.isTrash="0" and ad.section_id='.PlaceAnAd::RENT_ID.' ) as rent_total
        ...
        public function actionDeveloperDetails($id=null){
            $criteria=new CDbCriteria;
			$criteria->select = 't.*,cn.country_name
			,(select COALESCE(count(ad.id),0) from {{place_an_ad}} ad where t.user_id = ad.user_id and ad.status="A" and ad.isTrash="0" and ad.section_id='.PlaceAnAd::SALE_ID.' ) as sale_total
			,(select COALESCE(count(ad.id),0) from {{place_an_ad}} ad where t.user_id = ad.user_id and ad.status="A" and ad.isTrash="0" and ad.section_id='.PlaceAnAd::RENT_ID.' ) as rent_total
	 ...
	 public function actionBloglist($offset=0, $limit=10 , $id = 20 ){
		...
		if(!empty($model)){
			...
			foreach($model as $k=>$v){
				...
				$list[] = array('id'=> $v->article_id, 'title' => $v->title, 'image' =>@$imges['1'], 'short_description' =>$string , 'post_date' => $date ) ; 
			}
    ...
    public function actionList_customers($user_type='A',$country_id=null,$state_id=null,$limit=10,$offset=0,$logged_in=null)
     {
				...
				if(!empty($field)){	
				    ...
				    foreach($field as $k=>$v){
				       switch($user_type) {
                        case 'C':
                        $agent_list[] = array( 'user_id'=>$v->user_id,'full_name'=>$v->fullName,'nationality'=>$this->checkEmpty($v->country_name) , 'image' => Yii::app()->apps->getBaseUrl('uploads/images/'.$v->image,true) , 'short_description'=>$v->ShortDescription)  ;
                        break;
                        case 'D':
                        $agent_list[] = array( 'user_id'=>$v->user_id,'full_name'=>$v->fullName , 'image' => Yii::app()->apps->getBaseUrl('uploads/images/'.$v->image,true), 'short_description'=>$v->ShortDescription )  ;
                        break;
				        default:
                        $agent_list[] = array( 'user_id'=>$v->user_id,'designation'=>$this->checkEmpty($v->designation),'nationality'=>$this->checkEmpty($v->country_name),'full_name'=>$v->fullName , 'image' => Yii::app()->apps->getBaseUrl('uploads/images/'.$v->image,true) , 'short_description'=>$v->ShortDescription )  ;
                        break;  
